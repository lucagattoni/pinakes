...
| 9 | ~~**X7** — line 3's three layers~~ — **LAYER 2 BUILT 20260826**, landed `6a77f3c`: `line3 > R` → marker required, `== R` → forbidden, `< R` → red, row unreadable → **hard fail**. 54 mutants, 0 survived. **Layer 3** (the PyPI index query as its own soft CI job) is **not built** and is what remains of this row | — | coder |
| 10 | ~~**X7 doc half**~~ — **BUILT 20260826 07:24 UTC.** `docs/RELEASING.md`'s line-3 sweep row now **asks for the marker** — it never did, which is why the marker was 0-for-2 — with its shape, its `R`, the rule that the qualifier **names the index and never the tag**, when to remove it, and layer 2's own stated limit. The `docs/VERIFICATION.md:787` half was **discharged by layer 2 itself**: the false docstring (*"the invariant holds with no exception window"*) was rewritten in `6a77f3c`. **And a live defect found while writing it** — `docs/STATUS.md` line 3 carried *"NOT tagged and NOT on PyPI"*, a claim about **git** that goes false at `git tag` while the version is still unpublished, with the gate green over it because `HOLD` only requires ⏸ plus a bold span naming `R`. Aligned to the decided shape. **Gate driven red on purpose to prove it sees the marker: exit 1 without, exit 0 with** | — | planner |
