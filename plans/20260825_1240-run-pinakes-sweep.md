...
| 4 | **S1** — `PermissionError` aborts the whole walk · ✅ **BUILT 20260902 09:33** — six commits, `3f6a3ed`..`665ba31`, all ancestors of `origin/main`. `sync.py:684-692` catches `OSError` around the **hash**, not the walk; `docs/VERIFICATION.md:1361` carries a 13-row section; `pytest -k "unreadable or held"` → 25 passed. **The measurement below was true when taken and false 22 hours later**, and it is left standing as the record of that: a `✓ measured` stamp says when a fact stopped being current, not that it still is. Superseded text: — `sync.py:693` calls `hash_file` inside a `try` that catches `PinakesError` only, and `PermissionError` is an `OSError`; `cli.py` `main()` catches only `PinakesError`. The one `PermissionError` test (`test_sync.py:1727`) exercises `create_sidecar` on the *write* path, not the walk | nothing | coder |
| 5 | **S4** — escape at render in `template.py` · ✅ **BUILT AND LANDED `51e7731`, 20260903, shipped in 0.32.1.** The fix is `finalize=` on the Jinja `Template` in `src/pinakes/template.py`, so it covers **every** interpolated value rather than `--name`, and both ways in — `init` falls back to the directory's own name. **It does not only escape**: an unpaired surrogate, which is what POSIX makes of any invalid UTF-8 byte in an argument or a filename, has no TOML form raw *and* none escaped, so it is **refused** before anything is created, with a message naming the code point and never echoing the value. That case reproduced S4's own end state — zero-byte manifest, `init` then refusing the directory — **on the branch that fixed S4**, and it is rowed as row 23 of the live build order. Shipped with **37 tests**, **14 `docs/VERIFICATION.md` rows** and a new `tools/batteries/src-pinakes-template.toml` at **10/10**. **What it deliberately does not do**: escaping for TOML discharges nothing a terminal, a filename or a log line is owed — `pnk budget` prints `kb.name` raw, which is row 25 — and a KB already bricked by this stays bricked, because the fix is at creation. Was **✓ STILL OPEN, re-measured 20260902 11:29** by *running it*, not by reading the source: `init --name 'Bob'\''s "Special" KB'` exited 0 and reported success while writing `pinakes.toml:2` as `name = "Bob's "Special" KB"`, which is broken TOML; the second `init` then refused with *already a KB*. **It shared row 4's stamp and row 4's stamp was false**, so it was re-checked rather than inherited. Original measurement, 20260901 11:35 UTC: `template.py:209-223` `_render` interpolates raw through Jinja with no escaping; `templates/notes/pinakes.toml.j2:2` is `name = "{{ name }}"`; no test constructs a name containing a quote | nothing — landed | coder |
| 6 | **S5–S9** — the accept-then-mishandle batch. **D-37 is what was stopping it**, at S6, and D-37 is answered · ✅ **BUILT AND LANDED — `79e5a88`, 20260903 11:53 UTC**, as **four** fixes rather than five: S8 and S9 are one missing check and it belongs at the boundary (`e38539d`), then S5 (`f111c7c`), S6 as D-37 **option E** (`dfa23de`), S7 (`85ac2d8`). The live build order's row 7 carries the full account. **This cell read `✓ measured still open, 20260901 11:35 UTC — all five unfixed` until 20260903 12:30**, naming `cli.py:333`, `sync.py:1108` and `sync.py:283` — line numbers that were true when they were measured and false for the 37 minutes after the landing. **It is the second row to go stale off that single measurement.** The first is recorded in the live plan's row 4: the same 20260901 11:35 stamp on S1, still reading *open* 22 hours later, on the strength of which a planner handed a coder shipped work as top priority. **A stamp records when a measurement was taken, not that it still holds — and the stronger it reads, the less anyone re-runs it.** One measurement, two rows, both left behind by the same landing week; the fix for the class is row 24 of the live plan, not another re-measurement. | nothing — landed | coder |
