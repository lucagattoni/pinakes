...
| 5 | **S4** — escape at render in `template.py` · 🔨 **IN FLIGHT — held by branch `20260902_1129-s4-escape-at-render` at `7217be9`, unlanded as of 20260902 12:04 UTC; check `git ls-remote` before taking it.** Was **✓ STILL OPEN, re-measured 20260902 11:29** — and this time by *running it*, not by reading the source: `init --name 'Bob'\''s "Special" KB'` exits 0 and reports success while writing `pinakes.toml:2` as `name = "Bob's "Special" KB"`, which is broken TOML; the second `init` then refuses with *already a KB*. **It shared row 4's stamp and row 4's stamp was false**, so it was re-checked rather than inherited. Being built 20260902 by the coder. Original measurement, 20260901 11:35 UTC: — `template.py:209-223` `_render` interpolates raw through Jinja with no escaping; `templates/notes/pinakes.toml.j2:2` is `name = "{{ name }}"`; no test constructs a name containing a quote | nothing | coder |
