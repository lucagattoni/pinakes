...
> described the tree.** *(The count is left as it was measured. **A thirteenth was found on
> 20260830** — row 11, which read `BLOCKED` / `user-decision` on D-31/32/33 four days after the user
> answered them. It is the one row a reader would treat as a stop, and the reconciliation missed
> exactly that one.)* Eleven stated *LIVE* (eight) or *UNCLEAR* (three) for work that was **built, answered,
...
>
> **The mechanism, which is worth more than the list.** This file holds **two registers of the same
> facts** — this table, and § *Open questions* below. The 20260825 18:16–18:41 pass updated the
> bullets and never touched the table. **Nothing connects them**, so one evening was enough to make
...
>
> **✅ That planner pass ran 20260831 22:28 UTC, and all eight are now dispositioned.** Four are
> **parked** in the sweep plan's § *Decided work with an owner and no build order* — 12 as **one**
> schedulable unit rather than 34 rows, plus 13, 17 and 22 — which is the mechanism this file
> argued for and then did not apply to itself. Two are **not queueable by an agent**: 14 reduces to
> *which release should own `pnk adopt`*, a question `docs/ROADMAP.md` declines to answer, and 24
> needs the user's own material. Two are **re-marked ⏸ DEFERRED rather than BLOCKED** — 26 and 27
> are the eval-gated staged channels, which `CLAUDE.md` files as *never scheduled*, so BLOCKED
> implied a queue they were never in. The adversary said so on 20260825 and the cells did not move.
>
> **What this row cost, stated plainly:** the eight sat here five days because *no build-order row*
> was recorded as a finding rather than acted on as a disposition. Naming a gap does not close it.

| # | Where | What | Status | Blocked on | Owner |
|---|---|---|---|---|---|
| 1 | `plans/20260825_1240-run-pinakes-sweep.md` — *S2 — `pnk doctor` reports a KB fully healthy after a sync silently dropped a document* | A sync `IntegrityError: UNIQUE constraint failed: documents.path` leaves the row at `state='deleted'` with sidecar and source intact on disk; the document is unfindable and `doctor` exits 0 — make doctor detect it. | **✅ BUILT 20260826 04:06 UTC** | — landed `3876b57`, text corrections `325ab9e` 04:35. **Verify by opening `src/pinakes/doctor.py:463 _retired_documents`**, not by reading this row | coder |
| 2 | `plans/20260825_1240-run-pinakes-sweep.md` — *S3 — `pnk serve` reuses one `sqlite3.Connection` across OS threads* | Give `serve` a per-thread connection: `serve.py:109-137` caches one `store.connect_ro` handle on the instance, and any pause over ~10 s between MCP calls (the normal agent session shape) fires a SQLite cross-thread error — back-to-back calls never do. | **✅ BUILT — `e526e29`, 20260831 22:22 UTC** | nothing | **coder** |
| 3 | `plans/20260825_1240-run-pinakes-sweep.md` — *S1 — `pnk sync` aborts the entire index on one unreadable file* | Handle `PermissionError` in the walk: `sync.py hash_file` is a bare `path.read_bytes()`, so one `chmod 000` file raises an unhandled traceback, exits 1, and builds no index database at all — nothing else in the KB becomes reachable. | **✅ BUILT — six commits `3f6a3ed`…`665ba31`, 20260902 09:33 UTC; shipped in 0.32.0** | nothing | **coder** |
| 4 | `plans/20260825_1240-run-pinakes-sweep.md` — *S4 — a KB name that is not valid TOML bricks the KB at creation, silently* | Escape at render in `template.py:_render` (settled between two sessions — not reject at `init`): a name containing `"`, `\` or a non-tab control character makes `init` exit 0 printing *created* while every later command exits 1, and `pnk init` refuses to repair it. Needs no flag — the directory name reaches the same path via `init.py:355 root.name`. | **✅ BUILT — landed `51e7731`, 20260903 10:16 UTC; shipped in 0.32.1** | nothing | **coder** |
| 5 | `plans/20260825_0749-exposure-and-silent-status.md` — *X7 — line 3's three layers (D-35 answered 20260825 12:37)* | **Layers 1 and 2 are BUILT; layer 3 alone remains.** `tools/status_header_gate.py` already holds `_check_hold_marker`, `PUBLISHED_ROW` and the `SEQUENCES` import from `release_order_gate.py` — that is layer 1 (`line3 == __version__`) and layer 2 (NEW/offline/hard: compare against the *Published versions* row tail; marker required/forbidden/red, row unreadable = hard fail). **What is left is layer 3**: the PyPI simple-index query as its own soft CI job — nothing in `.github/workflows/ci.yml` or `release.yml` queries the index today. **Do not rebuild layers 1–2.** This cell read *"Build the decided three-layer gate … LIVE"* from the build until 20260831 22:28 UTC, which is how a landed increment gets built twice. Read the section, not the memory of it — the three layers are not what the plan first proposed. | **PARTLY BUILT — layer 3 only** | nothing — D-35 answered, explicitly unblocked | **coder** |
| 6 | `plans/20260825_1240-run-pinakes-sweep.md` — *Medium (S5, S6, S7, S8, S9)* | Five accept-then-mishandle defects: `--sidecars-only` with `--index-only` writes a sidecar while reporting all zeros; the *moved without its sidecar* message fires on ordinary deletion naming a file that no longer exists; the failure ledger never clears even when the document is repaired, and its remediation text is wrong; `-k -1` is a raw negative-slice bound returning 19 passages and `-k -100` prints *no passages matched* at exit 0; `pnk ask -k -1` raises an unhandled `ValueError` traceback from `deep/estimate.py:456`. The plan asks whoever builds them to say whether S1/S5/S6/S8/S9 are one fix or five. | **✅ BUILT — landed `79e5a88`, 20260903 11:53 UTC. Four fixes, not five: S8 and S9 are one check at the boundary** | nothing | **coder** |
| 7 | `plans/20260825_1240-run-pinakes-sweep.md` — *D-36 — `pnk link` detaches a trailing comment from an alias-reference line* | Decide: defect to fix, or a missing bounds row in `docs/MANIFEST.md` plus a pinning test. `sameTags: *commontags  # comment` splits into two lines with the comment orphaned. Not on MANIFEST.md:307-319's ten exclusions; MANIFEST.md:303 promises comments survive and VERIFICATION.md:282 pins it; none of the six anchor/alias tests covers it. Bears on the sidecar byte-identity invariant — read `docs/INVARIANTS.md` first. | **✅ ANSWERED 20260825 18:16 — option E** | nothing. Build is **row 8 of the sweep plan's build order** as of 20260826; it had no queue position before that. **Option E was invented by the adversarial pass and overturned the first pass's recommendation — read the decision, never the two options this row's text names** | coder |
| 8 | `plans/20260825_1240-run-pinakes-sweep.md` — *D-37 — what should S6's state *mean*?* | Decide what *source gone, sidecar present* means — a move whose other half has not been seen yet, or a deletion. Choosing the wording is implementation; choosing the meaning is not. | **✅ ANSWERED 20260825 18:16 — option E** | nothing. Build is **row 9 of the sweep plan's build order**. Gate the move hint on the **orphaned sidecar**, not the mint count — also invented by the adversarial pass. **This was the only decision blocking queued work** (S6), so the S5–S9 batch now runs to completion | coder |
| 9 | `plans/20260805_1313-decisions-init-titles-and-grammar.md` — *Compatibility: it sets a `requires_pinakes` floor, explicitly* | Reinstated by the adversary and confirmed by me: this decision's *Required* clause is undischarged and in unrecorded conflict with D-11/D-6. Nothing in Pinakes ever makes a KB carrying `[chunking] headings` or `metadata` declare a floor — `init.py` never writes `requires_pinakes`, and `upgrade.py:1264` only prints, never writes (D-11, pinned by `test_requires_pinakes_is_never_written`). The user-visible consequence is the exact misdiagnosis the field was built to prevent: a 0.6.0–0.12.x build opening such a KB reports *unknown key*. Decide whether the decision is withdrawn or discharged. | **CLOSED-SUPERSEDED 20260825 18:16** | nothing — **superseded, deliberately not withdrawn**: the clause's premise failed and the general question is folded into `docs/KB-UPDATES.md` § 8, which has held it since 20260728. **A coder residue survives** — `_toml.py`'s unknown-key remedy — and it is now rowed in the sweep plan's § *Decided work with an owner and no build order* | coder (residue only) |
| 10 | `plans/20260804_1844-decision-parent-child-arity.md` — *The decision* | Requirement 3 is open and its trigger has fired: *if the ceiling is alarming, the immediate-parent variant is the arm to measure*. The ceiling was measured at 53.42 `parent-child` rows/chunk, 94.7% of all stored edges, +113.4% index growth. No immediate-parent arm exists — `tools/graph_matrix.py` LEGS has seven legs and none is one; `grep -rE 'immediate.parent\|immediate_parent' src/ tools/ tests/` returns one prose line in `tests/test_edges.py:1162`. Requirements 1 and 2 are discharged. Tracked in this file alone. | **✅ CLOSED 20260825 18:16** | nothing — **the conditional's antecedent is measured false on every real corpus** (4.95 rows/chunk here, 3.80 over 300 real specifications, 0 of 300 reaching the synthetic 53.42). Transitive stays as built, no arm is owed, and nothing in `src/` changes | — |
| 11 | `plans/20260731_1202-open-corrections.md` — *`pnk init`'s gitignore warning is the only thing keeping a KB's `.pinakes/` out of a repository, and E5 raised what is in there* | The `init` half shipped as X1 (`init.py:122 _ignored_by_git`, `:303 pinakes_tracked`); the recurring `doctor` check has not been built. **D-31, D-32 and D-33 were ANSWERED by the user 20260825 18:16 — Option C, `pnk doctor` asks *both* questions (tracked *and* ignored), *unconditionally*; the brief's own `ls-files`-first shortcut was overturned as a regression** (`20260825_1803-open-decisions.md:20`). Nothing is undecided; the text an implementer may not choose has been chosen. | **DECIDED, not built** | nothing | coder |
...
| 13 | `plans/20260804_1016-template-release.md` — *F3 — `docs/KB-UPDATES.md` §3 case 1 describes something that never happened* | Four rotted code citations assigned to T4; T4 shipped in 0.20.0 without them. Verified still wrong: `store.py:205` (twice), `doctor.py:205`, `sidecar.py:35,106`, `_toml.py:184` — find them with `grep -n 'store.py:205\|doctor.py:205\|sidecar.py:35\|_toml.py:184' docs/KB-UPDATES.md` rather than by line, since these have moved twice. Real locations are `_check_schema_version` in `store.py` and `_template` in `doctor.py` — **by symbol, because this sentence's own line numbers rotted too** (it said `store.py:250` and `doctor.py:236`; they are `:259` and `:238` as of 20260901). Note the §3 citation was already re-derived once and has rotted again. F3's headline claim is separately closed — KB-UPDATES.md:56-61 now withdraws it — so the heading no longer describes what is open. | **LIVE** | nothing | **planner** |
| 14 | `plans/20260804_1016-template-release.md` — *D-8 — Is `pnk adopt` in this release?* | The only decision in this file nobody answered, and the disagreement it exists to end is still running: `docs/graph/PINAKES_APPROACH.md:420` assigns `pnk adopt` to the template release while `docs/README.md:28` files it under *What is still a proposal*. The template release closed at 0.22.0 with both documents standing. Either drop the APPROACH row or add the work. | **LIVE — but not the question this row names** | nothing. **Noted rather than repaired 20260825 18:44**: `docs/graph/PINAKES_APPROACH.md:423` now carries a ⚠️ recording that `pnk adopt` does not exist and that the template release closed at 0.22.0 without it. What stays open is **which release should own `pnk adopt`** — a question that table explicitly declines to answer. So the ask is no longer *drop the row or add the work* | planner |
| 15 | `plans/20260727_1543-v0.2.md` — *Decisions taken (decision 12) / I7c — The completeness audit, staging, and all-or-nothing commit* | An orphaned deferral whose trigger fired. Decision 12 and I3b both say the `word_coverage` floor and the paid re-extraction loop land in v0.3 *fitted on a real distribution*. 0.3.0 shipped without them; `extract/audit.py`'s docstring still opens *report-only in this release* and `floors.toml` carries no `word_coverage` key. The real paid output the deferral waited for has existed since the human-gated run of 20260729. `grep -rn 're-extraction loop' plans/ docs/` hits this file only — no ROADMAP row, no STATUS row, no owner. Schedule it or record a decline. | **⏸ DEFERRED 20260825 18:16, with a trigger** | **the trigger text, which is not in the code yet.** Its home is `src/pinakes/extract/audit.py`'s docstring — read by whoever would build it, not by a sweeper — and **verified 20260826 that the docstring does not carry it**. Planner dictates the text, coder pastes it unchanged. Rowed in the sweep plan's § *Decided work with an owner and no build order* | planner → coder |
| 16 | `plans/20260805_1313-decisions-init-titles-and-grammar.md` — *What this does not decide* | Whether to re-run the graph gate after 0.13.0's chunker fix is unanswered anywhere — not in `plans/`, `CHANGELOG.md` or `docs/STATUS.md`. Not academic: the gate's null verdict was measured on an index where all 106 806 chunks had an empty `heading_path`, and the corpus's committed manifest still carries no `headings` key. Costed at ~2 h CPU embedding plus a `schema_version` 3 rebuild, and carries the anti-circularity discipline in full. | **✅ ANSWERED 20260825 18:16 — run it, later** | nothing. Its own three-leg gate, **no immediate-parent eighth leg**, ~2.4 h unattended CPU, expected result another null. **It had an owner and no build-order row for 21 days; as of 20260826 it has one** in the sweep plan's § *Decided work with an owner and no build order* | coder, then planner |
...
| 18 | `plans/20260821_0745-mutation-harness.md` — *(H1 preamble — invisible to a `##` sweep)* | Two named fields were specified-as-decisions and never built: `expect_green` (T4's *control that had to stay green* column) and a per-mutant zero-kill allowance. `grep -rn expect_green` hits only this plan; `tools/mutate.py:1199` has the batch-level `--allow-zero-kills` only; `tools/batteries/README.md` documents `file`/`old`/`new`/`kills` and no green control. Nothing in CHANGELOG or docs records a decision either way. Every `##` section of this file is closed-built — this is the only residue and a heading grep cannot reach it. | **❌ DECLINED 20260825 18:16** | nothing. **Measured, not argued**: the field is in two commits and no code; 0 of 146 mutants across seven batteries asks for a green control; `load_battery` rejects no unknown key, so one written today would be **parsed, ignored, and reported to nobody** | — |
| 19 | `plans/20260825_0749-exposure-and-silent-status.md` — *X5a/b/c, X6 — write the four conventions down* | Four conventions this repository has re-derived and never written down, plus the invisible-handoff item. Listed at build-order row 7, blocked on nothing, planner-owned. | **✅ ALL FOUR BUILT 20260825** | — X5b at 12:52 (`docs/RELEASING.md`); X5a, X5c and X6 at 13:00. The user chose the **slim** form for `CLAUDE.md` — a pointer, not the rule inline — so `docs/BUILDING.md` carries § *Content mine, keystrokes yours* and the `RESUME.md` bound | planner |
| 20 | `plans/20260825_0749-exposure-and-silent-status.md` — *X7 doc half — `docs/RELEASING.md` sweep row, `docs/VERIFICATION.md:787`* | Two documentation debts D-35 named explicitly: `docs/RELEASING.md` never asks for the hold marker at all (a release cut by following it verbatim produces the false line — the marker is 0-for-2), and `tools/status_header_gate.py`'s docstring lines 1 and 10-13 are already literally false on today's tree (*'The invariant holds with no exception window'* — there is one, used deliberately three times). The docstring half is owed regardless of what was decided. | **✅ BOTH BUILT 20260826 07:24 UTC** | — the false docstring was rewritten by layer 2 itself (`6a77f3c`), and `docs/RELEASING.md`'s line-3 sweep row now asks for the hold marker, its shape and its removal. Build-order row 10 of the exposure plan | planner |
| 21 | `plans/20260825_1240-run-pinakes-sweep.md` — *Low* | Symlink loop invisible to `sync` and `doctor` (verifier downgraded high→low: a loop resolves to no content, ordinary Unix semantics); `-k 0` silently ignored on `search` and `ask`; `--source-type` accepts any string unvalidated so a typo reads as an empty KB; `confidence_reason` reports *nothing matched the filters* when no filters were given. | **LIVE — unblocked 20260903 11:53** | nothing — S1 through S9 are all built (`e526e29`, `3f6a3ed`, `3876b57`, `51e7731`, `79e5a88`) | **coder** |
| 22 | `plans/20260807_2143-docs-audit-findings.md` — *(audit header — the deferred ROADMAP review)* | A deferral whose trigger fired and which no section carries, so it is invisible to a section-by-section sweep: the audit defers a full review of `docs/ROADMAP.md` *until after T2*. T2 shipped in 0.18.0. `docs/README.md`'s docs-audit row still records the review as 'still owed' (`grep -n 'still owed' docs/README.md` — the line number this cell used to carry had rotted). | **LIVE** | nothing — the trigger fired at 0.18.0 | **planner** |
| 23 | `plans/20260731_1202-open-corrections.md` — *`tools/fragments.py` validates the fragments it reads and never the document it writes* | A residual open question hides inside a CLOSED item. After *CLOSED 20260824 00:35* sits '**Left open deliberately: widening the body rule.** … Widening is two different decisions … **Neither is taken.**' Genuinely unbuilt: `fragments.py` sets `opened = None` after checking only the first non-blank line under a `###` heading, so the fourth bare-paragraph body shape is unreachable by the rule. Anyone treating the section as closed loses this. | **⏸ DEFERRED 20260825 18:41, with a trigger and an owner** | nothing mechanical. It is the **setext-plus-indentation pair, not widening 1** — widening 1 reaches one section written 20260725, cannot reach the class that shipped, and needs three carve-outs not two. **Both target classes measure zero live instances today.** Owner is the user, because which widening is wanted is a judgement about a rule that fires on nothing | user |
...
| 25 | `plans/20260804_1016-staged-channel-gates.md` — *What "not scheduled" means operationally* | The clause defines its own tripwire — *two entries exist and are deliberate; if a third appears, that is the drift this rule exists to catch* — and at least six PPR/`[ner]` mentions now exist across `docs/`. The substantive rule still holds: none is a build plan, increment or numbered item, and `docs/ROADMAP.md` § *The graph release, staged — gates only, not scheduled* says so explicitly (`grep -n 'There is deliberately no' docs/ROADMAP.md`). Only the enumeration is falsified. | **✅ RULED 20260825 18:41 — it has NOT fired** | nothing. **Clause 1 counted permitted entries and its count was false when written** — `docs/DESIGN.md` had carried a third entry of the same shape since 20260729, five days earlier. The clause now forbids a *class* (no plan, no increment, no numbered item, no version number) and states that index, routing, survey and naming entries are permitted and uncounted. `docs/ROADMAP.md` is obedience, not drift | — |
| 26 | `plans/20260804_1016-staged-channel-gates.md` — *Gate 1 — PPR (`graph_channel = "ppr"`) / The gate / The corpus the measurement needs* | Not runnable. Clause 1 is P2 — *`expand` is licensed on* — and it is false: G5's gate ran 20260804 22:52 with 0 improved, 3 regressed, p=1.0000, so `expand` ships `off` (`manifest.py:85`) and `"ppr"` is not an accepted value (`manifest.py:77`). The corpus clause fails independently: the heading floor is 50% of chunks carrying a non-empty `heading_path` and the RFC corpus scores 0%. `docs/ROADMAP.md`'s *A different channel design* bullet, under § *The graph release — shipped 0.11.0*, already records that G5's result licenses neither PPR nor `[ner]` (`grep -n "G5's result licenses" docs/ROADMAP.md` — the full phrase wraps across two lines and will not match). | **⏸ DEFERRED 20260831** — not BLOCKED: `CLAUDE.md` files the graph release as *eval-gated, never scheduled*, so this was never in a queue to be blocked out of | P2 (`expand` never earned its default), plus no corpus meeting the heading-coverage floor — and the 300-RFC corpus 'lived locally and died with the machine' per CHANGELOG 0.13.0 | **planner** |
| 27 | `plans/20260804_1016-staged-channel-gates.md` — *Gate 2 — the `[ner]` extra and `mentions` edges / N4 / N5* | Unbuilt at every level, not merely ungated: no `[ner]` extra in `pyproject.toml`, no `mentions` kind in `edges.py:101 ALL_KINDS` or the `store.py:176` schema CHECK, no `LegSpec` for it in `graph_matrix.py`, and no script implementing N5's bridging-pair pre-count. N4 — is a model-derived edge set a `schema_version` bump and a coherence field — is unanswered anywhere in the repository, and the plan says answer it before deriving a single edge. | **⏸ DEFERRED 20260831** — not BLOCKED, for the reason in row 26; the adversary called these DEFERRED on 20260825 and the cell did not move until now | N2 (no channel is licensed on, so `mentions` edges are inert by construction and the gate is unmeasurable). The adversary noted these read better as DEFERRED than BLOCKED, since N2 has no live path to being satisfied. | **planner** |

**Five status cells corrected 20260903, in the same pass that flipped the sweep plan's own row 6,
and the rest of this table was deliberately NOT audited.** Rows 2, 3, 4 and 6 read **LIVE** for work
that was not merely built but **published to PyPI** — S3 for three days, S1 and S4 for one release
each, S5–S9 from the moment they landed — and row 21 read
**BLOCKED** on *"behind S1-S9"* when all nine were built. Each new cell names the sha it was
verified against. **Rows 12, 13, 14, 17, 22 and 24 still read LIVE or BLOCKED and were not
checked**: settling them needs judgement about work this pass did not touch, and a wrong ✅ is worse
than a stale LIVE. `CLAUDE.md` already records that twelve of these 27 rows had stopped describing
the tree; **this pass fixes five of them and leaves the count honest rather than pretending to a
...
- **✅ CLOSED 20260901.** That row now states no count at all — *"This row deliberately states no count. It said 39 open, one fixed, which went stale"* (`grep -n 'deliberately states no count' docs/README.md`; it has been at :58 and :62 and is elsewhere again, which is why no number is written here). Its surviving half, the still-owed `docs/ROADMAP.md` review, is on the same line. As raised: `docs/README.md:58` is stale and it is an entry point a freshly-cleared session reads first: it says the docs-audit plan holds '**39 open documentation corrections**' and '**One is fixed**'. The plan's own 20260823 pass says 6 fixed and 34 remain. It also still records the `docs/ROADMAP.md` review as 'still owed' — correct, but the trigger (T2) fired at 0.18.0, so it should read as due rather than deferred.
- ~~**RESOLVED 20260901** — counted, and this correction was right: 40 at the audit, 34 remaining, the `39` wrong in two cells (both now fixed).~~ `plans/20260807_2143-docs-audit-findings.md` contradicts itself on its own arithmetic: line 17 says 'Still open — this file | **40**' and the `# Medium — 13` / `# Low — 27` dividers sum to 40, while line 44 says 'Open at the audit | 39' against 'Fixed since | 6' and 'Remaining | **34**'. 39 − 6 = 33. The 34 is right and the 39 is wrong. A planner reconciling counts burns a pass hunting a 40th finding that was never missing.
- **✅ CLOSED 20260901 — with one residue.** That sentence now reads *"was empty at 20260805 22:18 and is not today"*, and §7's heading carries a stale-marker too. **The residue:** the correction says *"one live item and one closed in place (checked 20260825)"* while `20260731_1202-open-corrections.md`'s own `## Live` heading reads *"three items live as of 20260826 07:02 UTC"* — a correction that went stale the day after it was written. As raised: `plans/20260805_1721-metadata-as-retrieval-context.md:950` states '`plans/20260731_1202-open-corrections.md` has been empty since 20260805 22:18'. Verified false: that file's own § Live says it refilled from E5 and it currently holds two `###` items, one of which forwards to the only plan CLAUDE.md marks as carrying scheduled work. Same section's heading misdates 0.14.0 work to 0.12.0.
- **✅ FIXED 20260901 — the finding was live, the counts in it were not.** `plans/20260731_1202-open-corrections.md`'s preamble opened *"None live."* while its own `## Live` heading carried live items — the file an implementer opens contradicting itself, which is the failure CLAUDE.md names by this file's name. The preamble now dates that claim to 0.21.1 and forwards to `## Live` as the register. **As raised this bullet quoted two counts and both went stale within the day** — it said *one* live item and quoted `docs/README.md` saying *'One live item'*; the file's heading has since read three and `docs/README.md`'s row has read *"Three live items."* **Neither number is restated here on purpose: `grep -n '^## Live' plans/20260731_1202-open-corrections.md` is the only current answer.**
...
- **◐ HALF-CLOSED 20260901 — the primary copy is repaired, the duplicate is not.** `plans/20260804_1016-staged-channel-gates.md`'s header now warns *"Citations were re-confirmed at d06ef7e … and have since rotted"* and instructs locating by heading text. **The false-confirmation sentence survives in `plans/20260804_1016-graph-remainder-reentry.md`** — `grep -n 'still resolve' plans/` finds it there and nowhere else, so the second half of this bullet is what remains actionable. *(This bullet cited it at `:44` and it is at `:45` — the citation rotted by one line inside the bullet that reports citation rot, which is why the grep is written out.)* As raised: `plans/20260804_1016-staged-channel-gates.md` asserts in its header that 'Citations re-confirmed at `d06ef7e` … all four `STATUS` line numbers here still resolve'. That sentence is now false — `docs/STATUS.md:260`→392, `:312`/`:342`/`:383`→`:506`/`:597`+ — as are its `src/` citations (`eval.py:574`→707, `search.py:333`→405). Same false-confirmation sentence appears in `plans/20260804_1016-graph-remainder-reentry.md:44`. The `docs/graph/` citations in both still resolve exactly.
...
- `plans/20260804_1016-template-release.md` carries four falsified measurements that other rows depend on: F2's 'No template drift has ever added or removed a key' is false since notes@1.2 adds `daily_eur` (which gives D-11's `requires_pinakes` recommendation path a real input for the first time); the repeated 'one archived version, so no diff is reachable from `notes`' is false since 0.24.0; M3's `[budget]` diff is superseded by today's `per_operation_eur = 2.00`; and its Risks table says `notes@1.0` denotes six contents where D-2b/F1/T1 all say eleven, and cites `tests/test_store.py:93` as a test that no longer exists. The plan also under-counts its own wrong measurements at eight; `docs/README.md`'s `20260804_1016-template-release.md` row has the corrected total, ten (`grep -n 'measurements or specs have been wrong' docs/README.md` — that table shifts on almost every landing).
- **✅ CLOSED 20260901, both halves.** `docs/CLI.md` prints `notes  1.2` now, and `docs/ROADMAP.md` reads *"so for such a KB the last step is still the user's own edit. **That is no longer *every* KB**"* — cite it by that sentence, not by a range: the bullet's `2500-2503` now holds unrelated text and the audit's own re-derivation has since moved again. As raised: Outside `plans/`: `docs/CLI.md`'s `pnk templates` example output still prints 'notes 1.1' six lines above a `--json` paragraph that correctly says 'notes@1.2'; and `docs/ROADMAP.md:2500-2503` still says 'the last step is still the user's own edit, and will be until the next template bump' — that bump landed in 0.24.0.
- **✅ CLOSED 20260901.** That row is struck now (`grep -n 'upgrade. + .--apply. + .tomlkit' docs/KB-UPDATES.md`), and §9's closing prose records that the *'remaining two'* sentence *"was wrong twice over"* — no unstruck row and no 'two' left to have a referent. As raised: `docs/KB-UPDATES.md` §9's cost table leaves '`pnk upgrade` + `--apply` + `tomlkit`' unstruck and closes 'The remaining two … Neither is assigned'. Both are built (0.19.0 and 0.20.0), and only one row is unstruck, so the prose's 'two' has no referent. A planner reading §9 concludes `pnk upgrade` is unbuilt.
- **✅ CLOSED 20260901.** That row now spells the pointer out and retracts it: the `open-corrections` item for `resolve_path`'s return shape is **CLOSED, do not apply** — "it was written here as *item 9*; that file has never carried numbers, so the pointer could not be followed — corrected 20260825 13:12". **The bullet's own evidence check has inverted:** `grep -n 'item 9'` now returns that line rather than nothing. As raised: `plans/20260801_0102-links-and-graph-log.md`'s 20260731 20:43 row points at 'open-corrections item 9'. That file has no numbering — `grep -n 'item 9'` returns nothing. The pointer cannot be followed.
...
- **✅ CLOSED 20260901.** `docs/README.md`'s routing table carries the G3 row (`grep -n 'decision-g3-go' docs/README.md`), and the row records its own history — that it had no row until 20260825 and was reachable only from links inside other plans. As raised: `plans/20260804_1442-decision-g3-go.md` is absent from `docs/README.md`'s routing table while both its sibling decision records have rows. The G3 go/no-go decision is reachable only from links inside other plans.
...
> file. **The ten settled** are seven answered by the user 20260825 18:16, **one that turned out to
> be already ruled on `main` all along** (D-13's T6 trigger, in `docs/ROADMAP.md`), **one ruled here by
> the planner** (the staged-channel tripwire — that ruling is *this* branch's work, not prior art),
> and one declined. **The single remaining stop is the last
...
- **✅ ANSWERED 20260825 18:16 — E+F.** The 20260805 clause is CLOSED-**superseded** (its premise failed: nothing writes the floor: **D-11** (✅ TAKEN 20260804) settled that `--apply` never does, and `init` does not stamp it either — the latter resting on **D-6, which is a standing *recommendation*, not a taken decision**) and folded into `docs/KB-UPDATES.md` §8 beside the older general question, D-6 and D-11 cross-cited. The user-facing remedy was never missing: `docs/GUIDE.md` § *Troubleshooting* already answers the collaborator case. Coder half owed: `_toml.py`'s unknown-key message must offer the second hypothesis and point at MANIFEST.md. The `requires_pinakes` floor for `[chunking] headings`/`metadata`, and its unrecorded conflict with D-11 ('--apply never writes it', ✅ taken) and D-6 ('init never stamps it', a recommendation rather than a taken decision). Settled by: a ruling on whether a hand-set non-template key must carry a floor, and if so which surface writes it. Until then a 0.6.0-0.12.x build opening such a KB reports 'unknown key' — the exact misdiagnosis the field exists to prevent.
...
- **❌ DECLINED 20260825 18:16.** Measured: the field is in two commits and no code; 0 of 146 mutants across seven batteries asks for a green control; `load_battery` rejects no unknown key, so one written today would be parsed, ignored and reported to nobody. Two of T4's five filled entries are not expressible as a green control at all. It is also not the guard for the one hole the README says nothing catches. Whether `expect_green` and a per-mutant zero-kill allowance are still wanted in `tools/mutate.py`, or were tacitly dropped when 0.29.0 made batteries committed artifacts. The plan calls them 'decisions, not omissions'; nothing since records a decision.
...
which on 20260825 produced a sweep's worth of defects in one afternoon
