...
## Open questions — **no longer every one a stop.** As of 20260825 18:41: **eleven answered, two deferred behind written triggers, one still open**

> **Read the bullets, not this heading** — that is this file's own finding, and it applies to this
> file. The single remaining stop is the last bullet, and **it is not a decision**: two external
> repositories were never fetched and no reviewer ran a gate, so no user answer closes it. Every
> other bullet carries its disposition inline.
