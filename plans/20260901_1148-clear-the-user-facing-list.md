...
| 14 | **A retro fragment's *second* `## ` heading is spliced unvalidated** — require the form, not the value | — new | row 12 | Found by the coder's row-12 review pass 7 and **deliberately not fixed there**: not a regression, and the obvious rule would have refused a real released fragment. Measured across **116** fragments ever added under `retro.d/`, exactly **one** carries two column-0 `## ` headings — `retro.d/20260805_1907-a-guard-that-could-not-fire.md`, whose second stamp `19:15` differs from its filename prefix `1907` **on purpose**, recording a second moment inside one incident. **Ruled 20260901, planner:** a second heading must carry a parenthesised `YYYYMMDD HH:MM` stamp ending the heading, in the same spelling as the first; it **need not equal the filename prefix** and **nothing constrains its value**. One instance licenses the *existence* of a later stamp and does not license a monotonicity rule, so none is written. `retro.d/README.md` gains the sentence at the same time — planner-owned, dictated, not drafted. |
