...
**Every row here is coder work — but two of them contain a choice.** Row 9 must decide where two
`doctor` helpers live; row 23 must decide whether `pnk upgrade` fails or degrades. **A *Blocked on:
...
| 5 | **S4** — escape at render in `template.py` — 🔨 **IN FLIGHT, DO NOT TAKE** | 5 | nothing | **🔨 Held by branch `20260902_1129-s4-escape-at-render` at `7217be9`, pushed and unlanded as of 20260902 12:04 UTC.** Built and pinned: the fix is `finalize=` on the Jinja `Template`, nine tests in a new `tests/test_template.py`, nine `docs/VERIFICATION.md` rows and a new `tools/batteries/src-pinakes-template.toml`. **Check `git ls-remote --heads origin` before assuming this is still true** — if the branch is gone it landed, and this row and the sweep plan's row 5 both need flipping. The defect: silent, permanent, and `init` refuses to repair it — exit 0 on a bricked KB. |
