...
| 17 | **One character in a filename silently exempts a fragment from the stamp gate** — narrow the exemption | — new | row 12 | Found by the coder's row-12 pass 8. The stamp arm exempts a fragment when `_STAMP = re.compile(r"^\d{8}_\d{4}-")` fails to match the stem, so `20260901-0710-x.md` — hyphen for the underscore — is **exempted entirely**, reproduced with a control (`20260901_0710-x.md` refused, exit 1; the typo accepted, exit 0). **The coder called this a new rule and it is not:** `changelog.d` already refuses both that shape and a prefix-less one (*"filename must be `YYYYMMDD_HHMM-<category>-<slug>.md`"*), same tool, same run — `retro.d` is the stream that omits the check. An inconsistency between two halves of one mechanism, so **§ 4 does not hold it and row 15's refusal is not a precedent** — that gated battery section format, where there was no defect. **Ruled 20260902, planner:** exempt only a stem that **does not begin with 8 digits**; a stem beginning with 8 digits must match `^\d{8}_\d{4}-` exactly or be refused naming the prefix. Licensed by measurement over every fragment path that ever existed — `retro.d` 131 total / 17 unprefixed, `changelog.d` 179 / 35, and **0 of those 52 begin with 8 digits**; all are pre-convention names (`l5b-…`, `added-…`). So it breaks nothing in 310 fragments and preserves the prefix-less acceptance `body_of`'s docstring records as deliberate. **Rejected alternative:** give `retro.d` the full format gate `changelog.d` has — closes the hole completely and breaks nothing *live*, but refuses a shape that was accepted on purpose, which is a larger change than fixing the arm that misreads it. **Honest residual, to be stated in the code comment:** 8 digits narrows the hole, it does not close it — `2026090_0710-x` (seven) is still exempt. |
| 18 | **No fragment gate validates that a filename's date is a date** — DEFERRED behind § 4, measured not guessed | — new | **§ 4** | Raised by the coder against row 17: `20261345_9999-x.md` begins with 8 digits and matches `^\d{8}_\d{4}-`, so under row 17's ruling it would demand its heading carry `(20261345 99:99)` — and would pass if it did. **Measured before ruling, four names through `changelog.d`'s format gate:** `20260901_0710` exit 0, `20261345_9999` (month 13, day 45, 99:99) exit 0, `20260230_2500` (Feb 30, 25:00) exit 0, `99999999_9999` exit 0. **Shape only; neither stream validates a date.** So row 17 leaves the two halves consistent rather than disagreeing, and the absurd case is acceptable: the gate's subject is *the stamp is the filename's*, a copy check, not a calendar check — a wrong-but-possible date (`20260901_0710` written on the 2nd) is equally uncaught, and impossibility is that same class made visible. **Deferred, and here the new-process argument does hold:** row 17 was exempt from § 4 *because the sibling stream already had the check*; **neither** stream validates dates, so adding it invents a rule rather than propagating one. Same treatment as row 15. **The discriminator is *does the sibling already do this*, not *is it a small change*** — two questions that look identical get opposite answers on exactly that. Propose again if § 4 returns B or *neither*. |
