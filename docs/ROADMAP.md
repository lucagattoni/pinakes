...
- **64 releases in 41 days** — **counted from `CHANGELOG.md`, not incremented from this
  ...
  **The fix was broader than the design that ordered it** — the implementer found a third shape,
  ...
  `minimum-python` had completed at the release commit and been read in its log, because a gate
  that ran green at an earlier commit has not run at this one — it triggers only on a push to
  `main`. **Its controls discriminate the fix rather than the version string**: on 3.14 the
  published 0.32.3 reports no orphaned sidecar for a present-but-unreachable document where the
  published 0.32.2 reports one under the `--prune` remedy, and on 3.13 the two exit 0 and 1.
  **What it does not fix is now the most severe thing open** — a `[sources]` root the process
  cannot read enumerates nothing, so every document under it leaves the index at exit 0, on both
  interpreters.
  ...
  **That is the second consecutive
  release to produce it, not the first**: `c7b0bd9` carried the same marker for five minutes on
  20260830, which is what closes out *"produced by the release procedure zero times out of two"* in
  [STATUS § Published on PyPI](STATUS.md#published-on-pypi). The gate that made it happen shipped in
...
- **[Open corrections](#open-corrections)** — **three live as of 20260826 07:02 UTC.** The list emptied
  for the second time when all four were decided and built in 0.22.0, and **refilled the next day
  from E5**: `pnk init`'s
  ...
  under adversarial review, one was **created** by the increment that closed another, one came
  from **generalising a fix** — asking where else the defect just repaired still lives — and one from
  **reviewing what a new surface inherits**, which is how T7 found `pnk init`'s half-created KB. (The
  fifth was missing from this sentence until 20260826, while the same file listed all five twice
  elsewhere.)
- **🛑 One thing is scheduled, as of 20260825: the exposure plan**
  ...
  *and* ignored — **unconditionally**. D-34 closed as *promises only*.
  ...
  the open-corrections list holds **three live items** — its *decision* is now taken (D-31 to D-33,
  20260825 18:16), so what is left there is queued coder work, joined by `pageyield.py`, which
  justifies its constant with an example the constant refuses, and by the paid re-extraction loop's
  deferral trigger awaiting paste into `src/pinakes/extract/audit.py`. **`make release-check` closed
  20260826 07:02** — it ran no gate at all for its whole life, three `echo`s with no comparison and no failure
  path, and is now four legs in `tools/release_tag_gate.py` where **absence is red**. T6 waits on a **trigger** (a queried KB past
...
| | | **[Open corrections](#open-corrections)** | • **Three live as of 20260826 07:02 UTC.** `pnk init`'s gitignore warning is printed once, and E5 put the user's verbatim question under `.pinakes/` — **its required text is no longer undecided** (D-31 to D-33 answered 20260825 18:16), so it is queued coder work rather than a decision<br>• Joined 20260825 by three more, **one of them now closed**: `pageyield.py` justifies `SCANNED_PAGE_FRACTION` with an example the constant refuses, **at two sites**; the paid re-extraction loop's deferral trigger, dictated and awaiting paste into `src/pinakes/extract/audit.py`; and **`make release-check`, which ran no gate at all — closed 20260826 07:02**, four legs with **absence red**, and `docs/RELEASING.md` steps 6 and 7 swapped so the tag is created, gated, then pushed<br>• The `fragments.py` item closed in a day — and had been closed **once before on a mechanism dated three days before the defect**, which is why it recurred twice<br>• Six on 20260808, two closed in 0.21.1, four in 0.22.0<br>• **Two of the last four were never forks — they were unchecked assumptions**, refuted by running the code they described<br>• Five routes in: building, reading, shipping, generalising a fix, and reviewing a new surface — none finds the others'<br>• An empty list means nobody has run Pinakes lately |
...
| D-20 | `pnk init --backend` (`st` or `light`), and three copies of a false claim that `init` "cannot see which extra you installed" |
...
**Everything else in this release is tooling, documents and a red `main`.** A money assertion compared a
...
## 0.32.5 — a permission problem reported as three other things · 20260904 08:49

**One condition, three wrong answers, and none of them mentioned permissions.** A document or a
partner knowledge base that is **present but unreachable** — the shape a symlink into a directory
without `+x` produces — made `pnk link` blame the user's own path, made `pnk doctor` report `none`
about a document it could not read, and made a readable-but-refused partner come back as one that
does not exist.

**Why all three at once: the same guard, dead in the same place.** Each site probed with
`Path.is_file()` inside a `try`, and each had an `except OSError` written to turn the refusal into a
useful message. That probe **raises on Python 3.13 and returns `False` on 3.14**, so every one of
those guards fired on the declared floor and nowhere else. Three commands, three separate comments
justifying a branch that no longer ran.

**The fix is one predicate and it is deliberately not the obvious one.** `paths.unreachable` already
existed, and it is `lstat`-based — it answers about a link's own entry rather than its target,
because `sync`'s retirement pass must not follow links there. **Reusing it was the planner's first
ruling and it was wrong**: on `docs/alias.md → ../private/real.md` with `private/` unreadable — the
shape behind the release-before-last's ULID defect — it answers `False` on both interpreters, so it
would have left 3.14 unfixed *and* regressed 3.13 by moving that document out of the one list that
named it. The implementer measured it and refused to build it. `unreachable_through_links` is the
sibling that follows the link, and its not-there set is defined as **what 3.13's `is_file()` treats
as not-there**, because *"what `pathlib` ignores"* names no fixed set: on 3.14 `is_file()` ignores
everything, including both refusals.

**`why_not_a_kb` is total now**, so the three call-site guards were **removed** rather than kept as
backstops. A guard that cannot fire is dead code that lies, and this release found five of them.

**The developer tooling half exists because two sessions landed over red gates on one day**, hours
apart, both while quoting the rule at each other. `tools/land.py` refuses to land a tree no
`./check.sh` run has certified — keyed to the **merged** tree, not the branch tip, because when the
default branch has moved the merge produces a tree neither side gated. That distinction was found
before the guard shipped, and the rate behind it went through three estimates over three different
populations before anyone counted the whole one: five of thirty-five merges, about 14%, **every one
of them landed while a second session was active.** There is no override flag; `--cleanup-only` is
exempt. **The guard's own landing was the first thing it policed.**

`tools/mutate.py` now names the interpreter its counts came from. A battery inherits whatever its
venv holds, so *"64 of 64 killed"* was an interpreter-dependent claim that read as an absolute one —
and the same battery reports one survivor above the floor, which is an **equivalent** mutant rather
than a gap, because both spellings of that predicate return `False` there.
...
**This heading deliberately carries no count.** It said *one live* until 20260825, when a third
item arrived and renaming it broke two in-page anchors — a count in a heading is a status claim
that takes the anchor with it when it rots. The live tally is in the list below and in
...
and **that file's own heading is a claim too — read the item bodies, not either heading.**
...
**The oldest of the four:** `pnk init`'s gitignore warning is the only thing keeping a KB's
`.pinakes/` out of a repository, it is printed once at creation, and E5 put the user's **verbatim
question** in there. Not a new class of exposure — an unprotected `.pinakes/` already commits
`index.db`, which holds every chunk of every document — so it is a warning to strengthen rather than
a hole to close. **Its required text is no longer undecided:** D-31, D-32 and D-33 were answered by
the user 20260825 18:16 — `pnk doctor` asks **both** questions, tracked *and* ignored,
**unconditionally** — so this is **queued coder work, not a decision.** **The other three were all added
20260825, and one has closed:** `pageyield.py` justifies its constant with an example the constant
refuses, at two sites; the paid re-extraction loop's deferral trigger awaits paste into
`src/pinakes/extract/audit.py`; and **`make release-check`, which ran no gate at all, closed 20260826 07:02** —
four legs, **absence red**, and the tag now created, gated, then pushed. Owned by
[`plans/20260731_1202-open-corrections.md`](https://github.com/lucagattoni/pinakes/blob/main/plans/20260731_1202-open-corrections.md),
