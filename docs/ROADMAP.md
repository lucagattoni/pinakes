...
- **[Open corrections](#open-corrections)** — **four live as of 20260825 23:21.** The list emptied
...
- **🛑 One thing is scheduled, as of 20260825: the exposure plan**
  ...
  *and* ignored — **unconditionally**. D-34 closed as *promises only*.
  ...
  the open-corrections list holds **four live items** — its *decision* is now taken (D-31 to D-33,
  20260825 18:16), so what is left there is queued coder work, joined by two corrections raised the
  same day — **`make release-check` runs no gate at all**, and `pageyield.py` justifies its constant
  with an example the constant refuses — and by a fourth, the paid re-extraction loop's deferral
  trigger awaiting paste into `src/pinakes/extract/audit.py`. T6 waits on a **trigger** (a queried KB past
...
**unconditionally** — so this is **queued coder work, not a decision.** **The other three, all added
20260825:** `make release-check` runs no gate at all; `pageyield.py` justifies its constant with an
example the constant refuses, at two sites; and the paid re-extraction loop's deferral trigger awaits
paste into `src/pinakes/extract/audit.py`. Owned by
[`plans/20260731_1202-open-corrections.md`](https://github.com/lucagattoni/pinakes/blob/main/plans/20260731_1202-open-corrections.md),
