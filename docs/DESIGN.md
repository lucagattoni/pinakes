...
| Path **present**, the document itself unreadable | **Held, never retired.** The walk carries the path out separately rather than dropping it, and pairing yields `Skip` before any branch that reasons from absence. Dropped, it arrives at the row below instead: a `chmod 000` would soft-delete a document still sitting on disk and `pnk sync` would print `1 removed` for a file nobody touched. A `failures` row with a `chmod +r` remedy, a non-zero exit, and the sidecar is **not** reported as orphaned — `--prune` would otherwise destroy a live ULID |
...
| Recorded extraction is **paid**, effective backend is **free**, hash **unchanged**, but the row was **retired** and the file has come back | A `failures` row, for the same reason as the *changed* case above — `SoftDelete` drops the chunks, so the paid text really is gone — but it names **`RETIRED`** and never *"content changed"*. The file came back **byte-identical**; saying otherwise asks the user to pay for a change that did not happen, which is the conflation the row below forbids by name. **The fuller answer is deliberately not taken**: letting a warm extraction cache revive it for free would need `pair()` to tell a revivable document from one being silently downgraded, and it cannot — `_paid_survivor_in_current_index` requires `state = 'active'`, so a revived row gets no index-side protection at all, and `WalkedSidecar` carries no provenance to decide it from. **The cost is recorded rather than hidden: a user whose extraction cache is warm still pays.** Decided 20260830 |
