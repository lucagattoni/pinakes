...
| File | What it is |
|---|---|
| [`20260901_1148-clear-the-user-facing-list.md`](../plans/20260901_1148-clear-the-user-facing-list.md) | **The live build order across plans — 26 rows as of 20260903, and no longer all coder.** Rows **21 and 22 are planner-owned** (the dogfooding on-ramp's design, and regenerating `CLAUDE.md`), and **rows 9, 23 and 25 each carry a decision inside the body of a row whose *Blocked on* cell reads *nothing*** — read the body, not only the cells. Drafted by a read-only survey session and **reviewed, revised and adopted by the planner 20260901 11:58**; its § *What the planner changed in review* lists the four revisions and why each was made. **It schedules work the other plans own and re-decides nothing** — rows 1-6 and 11-12 name a sweep-plan row by number rather than restating it, so where the two disagree the sweep plan wins. **Only ENG-1 (§ 2) is new**: `check.sh` invokes 11 gates and **7 have no mutation battery**, including every file on the paid path (`src/pinakes/paid.py`, `errors.py`, `deep/loop.py`) — so by this repo's own rule that *a run with no kills is a broken harness, not a clean bill*, the strongest assertion protecting the money invariant has never been shown able to fail. **§ 4 was ANSWERED by the user 20260902 — option E**: no freeze, plus a written re-measure trigger. It is kept for its reasoning and its conflict-of-interest note, and is not a question any more. **§ 5 records that the G5 re-run cannot revive PPR** — a null there leaves the graph channel exactly where it is |
