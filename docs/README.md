...
| [`20260731_1202-open-corrections.md`](../plans/20260731_1202-open-corrections.md) | Numbered corrections for the implementing agent; items are closed in place, never deleted. **Two live items**: `pnk init`'s gitignore-warning question (added by E5) — a decision, not a task, and the item says why it is not urgent — and `tools/fragments.py` validating the fragments it reads but never the document it writes. The list emptied on 20260805 and at 0.22.0 and **refilled within days both times**: an empty list means *nobody has run Pinakes lately*, never *finished* |
