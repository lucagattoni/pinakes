...
| File | What it is |
|---|---|
| [`20260811_1358-deep-release.md`](../plans/20260811_1358-deep-release.md) | **The live build order, and the only plan with *scheduled* unbuilt work.** `pnk ask` and `pnk ask --deep`: seven increments, E1 to E7, and **all ten decisions (D-21 to D-30) are taken — eight on 20260811 14:17 and two more at E3's boundary, and it is the authority for every one of them**, with the rejected options kept and costed. **E1 to E5 are built: `pnk ask` free ([CLI](CLI.md#pnk-ask)), the round estimator, the paid client, the loop — so [`pnk ask --deep`](CLI.md#pnk-ask---deep) answers, bounded and budgeted — and the [run transcript](CLI.md#the-transcript) every paid run leaves under `.pinakes/deep/`. E6 — the measurement run — is part-built and in progress: the plan's E6 status block records what is done and what is still owed, and it is the only increment that spends real money — read [MEASUREMENT-RUN.md](MEASUREMENT-RUN.md) before continuing it. Then E7, printed suggestions. The release name stays in the unbuilt-work table until that final cut (D-9).** Two of its measurements change what the older documents imply: the budget machinery is **already built** (so this adds the loop, not the machinery), and `[retrieval.confidence]` **ships commented out**, so the escalation gate DESIGN § 4.2 depends on exists on no KB a user creates — which is why E4 raised the default `[budget]` caps (D-30): the uncalibrated branch is the one a stock KB takes, and under the old `per_operation_eur = 0.30` even a one-round loop was refused. Written against `main` at `106d01f` — **re-run its § 2 before trusting a `file:line`** |
...
| [`20260731_1202-open-corrections.md`](../plans/20260731_1202-open-corrections.md) | Numbered corrections for the implementing agent; items are closed in place, never deleted. **One live item** (added by E5): `pnk init`'s gitignore-warning question — a decision, not a task, and the item says why it is not urgent. The list emptied on 20260805 and at 0.22.0 and **refilled within days both times**: an empty list means *nobody has run Pinakes lately*, never *finished* |
...
| [`20260821_0745-mutation-harness.md`](../plans/20260821_0745-mutation-harness.md) | **Proposed 20260821, not scheduled** — a committed mutation harness, `tools/mutate.py`: the `land.py` precedent applied to the mutation step, the procedure's other silently-failing class (more than a dozen invalid runs recorded, six of them the identical `git checkout` trap). One increment; blocks nothing, blocked by nothing |
