...
| File | What it is |
|---|---|
| [`20260830_0927-main-is-red-and-a-review-that-half-ran.md`](../plans/20260830_0927-main-is-red-and-a-review-that-half-ran.md) | **🛑 READ FIRST: `./check.sh` is RED on `main` and no commit did it.** `src/pinakes/budget/prices.toml` is `as_of 20260728 16:31`; `manifest.py:880` defaults `max_price_age_days` to **30**; the table went stale **20260827 16:31** and **25 tests** now fail — 18 in `test_cli_ask.py`, 5 in `test_extract_claude.py`, 1 each in `test_doctor.py` and `test_pdf_trace.py`. **Every other gate passes**, `ruff` and `pyright` included. **CI's green on `3712a7f` is dated 20260826 11:51 and nothing has run since.** Proved by advancing `as_of` and watching all 25 pass, then restoring. **Unblocking it needs no decision**: `DESIGN.md:811` states staleness is *deliberately not a CI gate*, so `tests/test_doctor.py`'s wall-clock assertion is itself the defect, and `tests/test_deep_loop.py:153` already carries the cure. **The user's half is the shipped one** — `as_of` has never once been refreshed and `docs/RELEASING.md` has no step that would, so every install refuses paid estimates 30 days after each release. **Also carries the state of a five-lens self-review that lost 26 of 52 agents to a session limit** — 51 raised, 21 verified, **30 never verified** — with five defects confirmed by hand, including a bare pipe character inside a code span that **truncates the row on GitHub — dropping `transcripts]` and the rest — while the published mkdocs site renders it correctly and in full.** Same source, two renderers, and CI checks the one that is fine |
