...
| [**RELEASING.md**](RELEASING.md) | *How do I cut a release?* Fragments, version bump, tag, verification, and the three documents a release stales |
...
| [**KB-UPDATES.md**](KB-UPDATES.md) | *What happens to a KB somebody already has when Pinakes changes?* Design note — **part built**: `requires_pinakes` (G4, 0.6.0) and template-drift **detection** (§6's gate, 0.17.0), `pnk doctor` reporting **how far** a template has drifted (T2, 0.18.0), **`pnk upgrade`**, which prints the lines themselves (T3, 0.19.0), and **`--apply`**, which adopts the hunks that fit (T4, 0.20.0). What is still a proposal is what its §8 leaves open, and anything that reaches beyond `pinakes.toml`. **`pnk adopt` is not from this note** — it has never appeared in it; the §8 that proposes it is [PINAKES_APPROACH.md](graph/PINAKES_APPROACH.md)'s *ClaudeKB: the first fleet*, it is unimplemented, and which release owns it is D-8 of [the template-release plan](../plans/20260804_1016-template-release.md), unanswered |
...
`plans/` holds more than one kind of file. **As of 20260901 11:58 three files propose work: the
ordering plan, the exposure plan and the run-Pinakes sweep, all at the top of the table** — re-derived
by reading all three build orders, not carried forward. **The ordering plan schedules the other two
rather than adding to them**, so where it and the sweep disagree about a row's state, the sweep wins:
it carries the dated measurement and the ordering plan carries a summary of it. **A third had one between 20260830 and 20260831**, the
...
| File | What it is |
|---|---|
| [`20260904_1027-development-paused-handover-and-evidence.md`](../plans/20260904_1027-development-paused-handover-and-evidence.md) | **⏸ READ FIRST. Development is paused and this is the entry point for resuming.** Written as both live sessions shut down, at the user's instruction that the state be on disk with no ambiguities. Carries the exact state at the pause (`main` sha, what is published, the six waiting fragments and **why the release was deliberately not cut**), how to resume in four steps, the open rows with the decision each carries, and **measured evidence for the proposed next steps** — the floor audit's full chain (18 raised → 5 survivors → **3 real** → **0 reachable**, the first false-positive characteristic this project has), two red-gate landings in one day, four landed rows that still read open, and the fragment mechanism re-measured on a fresh cohort at a 2.2 h median. **It also records what it could not establish**, including the kill rate, which needs seeded defects nobody has run |
| [`20260901_1148-clear-the-user-facing-list.md`](../plans/20260901_1148-clear-the-user-facing-list.md) | **The live build order across plans — 37 rows as of 20260904 01:48, and no longer all coder.** **18 are open**, of which **21, 22, 28 and 36 are planner-owned** (the dogfooding on-ramp's design, regenerating `CLAUDE.md`, where `doctor` reports an unresolvable symlink, and probing the ~41 unexamined `pathlib` sites). **Rows 30–37 were all added on 20260903**, the day 0.32.2, 0.32.3 and 0.32.4 were cut — 31 is closed and shipped, 33 and 37 are the cuts, and **32 carries four audit findings that are still unverified**, so read its caveat before building from any of them. **This cell read *30 rows as of 14:35* for eleven hours while the table held 37**, which is the drift it exists to prevent and the third time it has drifted this way. **This cell read *28 rows as of 13:13* while the plan already carried 29** — row 29 was filed at 14:24 and this count was not moved with it, which is the drift this table exists to prevent. Row 29 is the `pathlib`-spelling checker; **row 30 was written after its own work had landed**, the 3.13 crash and the `pnk doctor --prune` bug, found and built inside two hours with no queue position and held only by a 🛑 in `CLAUDE.md`. Rows **21, 22, 27 and 28 are planner-owned** (the dogfooding on-ramp's design, regenerating `CLAUDE.md`, the 0.32.2 cut — ✅ done and published — and when `pnk doctor` walks), and **rows 9, 23, 25 and 28 each carry a decision inside the body of a row whose *Blocked on* cell reads *nothing*** — read the body, not only the cells. Drafted by a read-only survey session and **reviewed, revised and adopted by the planner 20260901 11:58**; its § *What the planner changed in review* lists the four revisions and why each was made. **It schedules work the other plans own and re-decides nothing** — rows 1-6 and 11-12 name a sweep-plan row by number rather than restating it, so where the two disagree the sweep plan wins. **Only ENG-1 (§ 2) is new**: `check.sh` invokes 11 gates and **7 have no mutation battery**, including every file on the paid path (`src/pinakes/paid.py`, `errors.py`, `deep/loop.py`) — so by this repo's own rule that *a run with no kills is a broken harness, not a clean bill*, the strongest assertion protecting the money invariant has never been shown able to fail. **§ 4 was ANSWERED by the user 20260902 — option E**: no freeze, plus a written re-measure trigger. It is kept for its reasoning and its conflict-of-interest note, and is not a question any more. **§ 5 records that the G5 re-run cannot revive PPR** — a null there leaves the graph channel exactly where it is |
| [`20260831_2242-agent-spend-two-findings.md`](../plans/20260831_2242-agent-spend-two-findings.md) | **What agents cost here, and what each number is *not*.** (1) **`fable` is a selectable subagent model neither `CLAUDE.md` names**, priced at 2.00× opus-5 on both input and output — and it is **realised, not hypothetical**: 551 of 866 fable-5 requests ran inside a fan-out, 471 of 618 in Pinakes. **Its first version said `0 of 868` and that was corrected 20260831 23:20** — read the corrected table, never the summary row above it. The fix is one clause in the **user's own global `~/.claude/CLAUDE.md`**, so it is **a proposal, never a queued item**. (2) **A killed workflow loses every agent that had not returned** — 157 of 868 workflow-agent runs recorded no terminal row, but they sit in 22 of 55 runs and 33 runs lost nothing, so it is not a per-agent death rate; all 157 transcripts are on disk, so a resume **re-runs** them rather than losing them. (3) Offered and unacted: **boot context is 56k tokens and grew 35% since 20260728**. Numbers are re-derivable with `tools/agent_spend.py`, which **prints the population it read** |
| [`20260831_2115-stale-entry-points-audit.md`](../plans/20260831_2115-stale-entry-points-audit.md) | **An audit of what a cleared session reads first, preserved verbatim from the scratchpad it would have died in** — 46 of its 62 findings had no other home. **✅ ALL 62 ARE CLOSED** — 15 blockers (`6a86482`, each re-verified individually), 13 misleading (`802d40b`), 33 cosmetic (`1d90613`). **Nothing here is a worklist any more.** Its lasting use is the record of *how* the last 33 closed: none of its suggested fixes was pastable after eleven landings, so each was re-measured — eight were already fixed, five suggestions had themselves rotted, and three of the audit's own corrections had rotted again within the day. Its own limit is why the count is usable: *"62 is what survived an adversary told to refute, not ground truth."* **Line numbers have shifted — locate by content** |
| [`20260830_0927-main-is-red-and-a-review-that-half-ran.md`](../plans/20260830_0927-main-is-red-and-a-review-that-half-ran.md) | **✅ The incident is closed — read it for what it carries, not for what it was called.** `main` is green and **0.31.0 is published** (20260830); `prices.toml` is now refreshed at every release (`RELEASING.md` § *Before you start* step 3, which had never existed, which is why `as_of` had never once been refreshed). **What is still live in it: 22 findings nobody has ever checked** — § *The 22 nobody has ever checked*, severity **as raised** by agents whose claims were never tested, so read the column as *what an unverified agent asserted*. It also holds the record of a review that **lost 26 of 52 agents and reported `"status": "completed"`**, the anatomy showing the dead agents cost **4.69%** while the five that succeeded cost **71.4%**, and **eight wrong claims** that were each a valid inference over a population nobody stated — with the rule that would have caught them: **state the selector, not the set**. The **seventh and eighth, both 20260831**, are the ones that reached a document, sixteen minutes apart, the eighth *inside the correction of the seventh* — a borrowed denominator, then a corpus that counted the measuring sessions as data. Neither was caught by re-reading; both were caught by re-running — §§ *A seventh instance*, *An eighth instance* |
...
| [`20260825_1252-plans-sweep-findings.md`](../plans/20260825_1252-plans-sweep-findings.md) | **Every section of every file in `plans/` classified by reading its *body* — 317 sections, 20 files, and 93 heading/body mismatches.** Discharges X3. **A heading in this directory is wrong about one section in six**, which is why the earlier heading-only sweep dropped nine files and lost the arity plan's only outstanding work. Carries **53 proposed heading dispositions** (the input X2 needs), **19 corrections owed** — factual claims wrong on the tree, including one in this very table — and **13 open questions — ten settled, two deferred behind written triggers, one still open.** The stop is the last one and it is **not a decision**: two external repositories were never fetched and no reviewer ran a gate, so only doing it closes it. **This row states no other count deliberately** — the ones above it went stale within a day. **Its § *Actionable* table is a dated snapshot, not a queue** — reconciled 20260826 after **twelve of its 27 rows** stopped describing the tree, the dispositions that falsified eleven of them stamped between 20260825 12:52 and 20260826 04:35, and one row reading *S2 · LIVE · blocked on nothing* from S2's landing until a planner happened to look; where it and a `## Build order` disagree, **the build order wins**. **Read § *Three of this sweep's own claims did not survive checking* first**: the pass that produced the list also caught itself three times. **Two external repositories were not fetched**, so anything turning on the dogfooding KB's or the RFC corpus's live state is marked UNCLEAR rather than resolved |
| [`20260825_1240-run-pinakes-sweep.md`](../plans/20260825_1240-run-pinakes-sweep.md) | **Defects found by *running* Pinakes, none by reading it** — S1–S9 from a seven-surface sweep, and **S16, S17, S18 and S19 all from adversarially reviewing a fix**, which is the sweep's own lesson turned on itself. **This row deliberately states no total**: the file's section counts do not reconcile and its Low findings are prose rather than numbered — each re-run from a clean directory by a verifier told to refute it; five refuted and dropped. **Its `High` section** (four findings): swapping two documents' names crashes `sync` with an `IntegrityError` while `doctor` exits 0 and `search` answers from the wrong file (**S16, still live**); `sync` meeting one unreadable file raises an unhandled `PermissionError` and builds **no index at all**, so nothing in the KB is reachable; `doctor` exits **0** on a KB where a sync `IntegrityError` left a document at `state='deleted'` with sidecar and source intact (**S2, ✅ BUILT** 20260826 — `doctor.py:463 _retired_documents`); `serve` caches one `sqlite3.Connection` across OS threads, firing on **any pause over ~10 s** and never on back-to-back calls. **Its `Medium` section**, including a KB name that is not valid TOML bricking the KB at creation with **exit 0**, unrepairable by any `pnk` command and needing no flag. **D-36 and D-37 were ANSWERED 20260825 18:16, both on options the adversarial pass invented.** **Its § *Build order* is the live coder queue** — reconciled against the tree 20260826 — and its § *Decided work with an owner and no build order* parks decided work whose own plan has no queue, each row linking to the plan that owns the decision. **Read its § Provenance before acting on a row** — the markers are not uniform, and one finding was recovered after a verifier returned *no verdict* and a classifier defaulted it to *refuted*. **The paid-path invariant survived** adversarial testing, measured from a stripped subprocess |
| [`20260825_0749-exposure-and-silent-status.md`](../plans/20260825_0749-exposure-and-silent-status.md) | **Its build order is fully built out **except rows 5, 6 and 9 of `plans/20260825_0749-exposure-and-silent-status.md:613-617`, which are un-struck, owner coder, blocked on nothing, and whose `doctor` check does not exist in the tree** (`grep -n gitignore src/pinakes/doctor.py` returns one unrelated docstring line). D-31 to D-34 were ANSWERED 20260825 18:16** (see the open-decisions file at the top of this table — two taken options were invented by that pass's adversary and appear in no earlier plan). Two subjects, one failure: what Pinakes promises about `.pinakes/`, and status claims nothing gates. **§ X1 is BUILT** (`35cdc79`) — read it for what shipped and for the two constraints the increment had to correct in this plan. The measurement behind it: the 0.30.2 detector reports **protected** for a repository that is actively committing the user's verbatim questions, because its probes are opaque tokens and so are never in the index. A correct `.gitignore` does not untrack an already-tracked file, and **nothing in Pinakes says so anywhere**. D-31 (does `pnk doctor` carry a recurring check), D-32 (WARN vs OK-with-a-note), D-33 (ignored *here* vs *for everyone*) are E5's reserved decisions, restated because M2 inverts the argument; D-34 (does `docs/VERIFICATION.md` map every test or promises only) is new and unscheduled. Part 2 carries the `plans/` restructure, the sweep that read 11 of 20 files, and four conventions this repository has re-derived and never written down |
...
| [`20260731_1202-open-corrections.md`](../plans/20260731_1202-open-corrections.md) | Numbered corrections for the implementing agent; items are closed in place, never deleted. **Three live items.** (1, added by E5) `pnk init`'s gitignore-warning question. **It is no longer a decision — D-31, D-32 and D-33 were ANSWERED by the user 20260825 18:16** (`pnk doctor` asks both questions, tracked *and* ignored, **unconditionally**), so it is queued coder work; the authority is [`20260825_1803-open-decisions.md`](../plans/20260825_1803-open-decisions.md), not the exposure plan's original framing. **(2, added 20260825, coder-owned)** `pageyield.py` justifies `SCANNED_PAGE_FRACTION` with an example the constant refuses, **at two sites**. **`make release-check` was the third and is ✅ CLOSED 20260826 07:02 UTC** — it ran no gate at all for its whole life (three `echo`s, no comparison, no failure path) and is now four legs in `tools/release_tag_gate.py` where **absence is red**; `docs/RELEASING.md` steps 6 and 7 swapped, so the tag is created, gated, then pushed. **The exposure plan's § X1 supersedes *that* item's *"not urgent"*** — the hazard it did not consider is measured, silent and unrecoverable. **(4, added 20260825)** the paid re-extraction loop's deferral trigger, dictated for `audit.py`'s docstring. **Also read § X2 there before trusting this file's `## Live` heading** — an item's closure can sit in its body, and that has now cost two near-rebuilds. The list emptied on 20260805 and at 0.22.0 and **refilled within days both times**: an empty list means *nobody has run Pinakes lately*, never *finished* |
...
| [`20260807_2143-docs-audit-findings.md`](../plans/20260807_2143-docs-audit-findings.md) | **The open documentation corrections from the 20260807 audit** — a worklist for the planner, **not a build plan**. **✅ The worklist is closed — all 34 remaining findings were fixed 20260901, as one unit**, together with two the pass surfaced that nobody had raised: the root `README.md`'s fourth copy of a sentence `58adfe2` had already called false in three sibling docs, and a `docs/DESIGN.md` §6.1 example invoking a template that does not ship. **The arithmetic contradiction this row used to flag is resolved by counting**: the audit had **40** findings, not 39 — 42 `## ` sections less two bookkeeping sections, the `# Medium — 13` / `# Low — 27` dividers, and 34 open + 6 already closed all agree, and the `39` was simply wrong in two cells. Read the plan's § *✅ CLOSED 20260901* for the disposition; **the finding sections below it still read as raised, deliberately**, so do not re-open one from its own text. Each names the file, line, claim, truth, evidence and fix; each was verified by a second agent prompted to refute it. **One is fixed, and it is the one to read before leaving the rest**: `docs/STATUS.md:303`, the `0.15.1` row out of release order, closed 20260811 13:27. It sat verified and unworked for four days while three release sweeps added **four more** misplaced rows in the same class — the cost of a finding grows after it is verified, and `tools/release_order_gate.py` is what now stops that one recurring. The doctor-adjacent ones are cheaper after T2 (shipped 0.18.0), and the three currency headers are restamped last. It also **defers a full review of `docs/ROADMAP.md` until after T2** — out of scope for the audit itself, and still owed |
...
| [`<stamp>-decision*.md`](../plans/) | A decision record — rationale, not instructions. **Five of them**, each carrying the `YYYYMMDD_HHMM-` prefix every plan was renamed to; **two** are `decisions-` plural, so match the stem rather than a literal `decision-` |
| `20260725_1317-v0.1.md`, `20260727_1543-v0.2.md` | **Shipped — with one exception the plan's own heading names and this row used to omit.** Historical build orders, still cited by `check.sh` and `tools/paid_path_gate.py`. **`v0.2` is shipped across 0.2.0–0.4.0 *except decision 12*, whose paid re-extraction loop has never shipped and is DEFERRED** — its trigger's **home** is `src/pinakes/extract/audit.py`'s docstring — read by whoever would build the loop rather than by a sweeper — and **it is not pasted there yet**: the text is dictated and queued as a live item in [`20260731_1202-open-corrections.md`](../plans/20260731_1202-open-corrections.md) |
...
| What the agents building this repo cost | **[`tools/agent_spend.py`](https://github.com/lucagattoni/pinakes/blob/main/tools/agent_spend.py)** | run it — it prints the population it read. No document restates a figure: the answer moves daily, and a plan that quotes one closes |
...
6. **A [`changelog.d/`](../changelog.d/README.md) fragment** — one file, named
   `YYYYMMDD_HHMM-<category>-<slug>.md` (UTC, read off the clock), in the same commit as the code. **Never an edit to `CHANGELOG.md`**:
