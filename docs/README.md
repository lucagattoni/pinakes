...
| File | What it is |
|---|---|
| [`20260830_0927-main-is-red-and-a-review-that-half-ran.md`](../plans/20260830_0927-main-is-red-and-a-review-that-half-ran.md) | **🛑 READ FIRST: `./check.sh` is RED on `main` and no commit did it.** `src/pinakes/budget/prices.toml` is `as_of 20260728 16:31`; `manifest.py:880` defaults `max_price_age_days` to **30**; the table went stale **20260827 16:31** and **25 tests** now fail — 18 in `test_cli_ask.py`, 5 in `test_extract_claude.py`, 1 each in `test_doctor.py` and `test_pdf_trace.py`. **Every other gate passes**, `ruff` and `pyright` included. **CI's green on `3712a7f` is dated 20260826 11:51 and nothing has run since.** Proved by advancing `as_of` and watching all 25 pass, then restoring. **Nothing lands until the user picks a remedy** — refresh the prices, raise the limit, or pin the clock in those tests; only the last stops it recurring, and it restores `DESIGN.md` §5's intent that staleness is a runtime refusal rather than a build gate. **Also carries the state of a five-lens self-review that lost 26 of 52 agents to a session limit** — 51 raised, 21 verified, **30 never verified** — with five defects confirmed by hand, including an unescaped `\|` inside a code span **splitting GFM table cells on the published `STATUS.md`**, which `mkdocs --strict` exits 0 on |
