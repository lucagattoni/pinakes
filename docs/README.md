...
| File | What it is |
|---|---|
| [`20260901_1148-clear-the-user-facing-list.md`](../plans/20260901_1148-clear-the-user-facing-list.md) | **The live build order across plans — 12 rows, all coder, none blocked.** Drafted by a read-only survey session and **reviewed, revised and adopted by the planner 20260901 11:58**; its § *What the planner changed in review* lists the four revisions and why each was made. **It schedules work the other plans own and re-decides nothing** — rows 1-6 and 11-12 name a sweep-plan row by number rather than restating it, so where the two disagree the sweep plan wins. **Only ENG-1 (§ 2) is new**: `check.sh` invokes 11 gates and **7 have no mutation battery**, including every file on the paid path (`src/pinakes/paid.py`, `errors.py`, `deep/loop.py`) — so by this repo's own rule that *a run with no kills is a broken harness, not a clean bill*, the strongest assertion protecting the money invariant has never been shown able to fail. **§ 4 is a standing decision for the user and blocks nothing**; read its third reading and its conflict-of-interest note before the recommendation. **§ 5 records that the G5 re-run cannot revive PPR** — a null there leaves the graph channel exactly where it is |
