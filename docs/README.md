...
remain *proposed and not taken*. The sweep carries fifteen confirmed defects and two untaken
...
| [`20260731_1202-open-corrections.md`](../plans/20260731_1202-open-corrections.md) | Numbered corrections for the implementing agent; items are closed in place, never deleted. **One live item** (added by E5): `pnk init`'s gitignore-warning question — a decision, not a task. **Its decisions are now proposed as D-31 to D-33 in the exposure plan above, which is where to read them**, and that plan's § X1 supersedes this item's *"not urgent"*: the hazard it did not consider is measured, silent and unrecoverable. **Also read § X2 there before trusting this file's `## Live` heading** — an item's closure can sit in its body, and that has now cost two near-rebuilds. The list emptied on 20260805 and at 0.22.0 and **refilled within days both times**: an empty list means *nobody has run Pinakes lately*, never *finished* |
