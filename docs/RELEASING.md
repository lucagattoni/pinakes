...
|---|---|
| `docs/STATUS.md` — **line 3** | `**Latest release: x.y.z**`. **Missed by four consecutive release sweeps** (0.5.0 → 0.7.1) because this table did not name it, while the same sweeps updated all three rows below. It is the first line a reader sees and it contradicted the file's own tables in a public repo. Bump it with `__version__`, in the same commit. **And the hold marker — this table never asked for it, which is why it is 0-for-2.** `__version__` means *landed on `main`*, not *published* (D-35), so between the release commit landing and the version reaching PyPI, line 3 names a version `pip install` cannot get **on a page that deploys on every push**. While that is true the line carries ` — ⏸ **landed on `main`; `pip install pinakes` still gets <R>.**`, where **`R` is the newest entry of the *Published versions* row** — and the qualifier **names the index, never the tag**: *NOT tagged* goes false at `git tag` while the version is still unpublished, so the marker would half-lie with the gate green over it. **Remove the marker in the same commit that appends the release to *Published versions*.** `tools/status_header_gate.py` layer 2 decides all of this: `line3 > R` → marker **required**, `line3 == R` → **forbidden**, `line3 < R` → red, the row unreadable → **hard fail**. **Its limit, so its green is not read as more than it is:** the row may lag a publish, so layer 2 enforces removal at the moment the row is updated, not at publication |
...
| `docs/ROADMAP.md` | **Five places, and the last two were missed by five consecutive sweeps.** The summary table needs a row; Part 4 needs a `## x.y.z — <title> · <stamp>` section (the table's row links to its anchor, so the two are written together or the link dies); Part 5's *Open corrections* heading **no longer carries a count, and must not regain one** — it said *one live* until 20260825, when a third item arrived and renaming it broke two in-page anchors; a count in a heading is a status claim that takes the anchor with it when it rots. **Then the two prose blocks: `## Where things stand right now` — its stamp, its release count and its per-release-name state — and `## The template release`.** Added 20260805 18:02 after 0.12.0's sweep; the prose blocks added 20260811 12:26 after they sat three releases behind while every table in the same file was current.<br>**`## Where things stand right now` is the one place in this table that belongs to the post-publish sweep rather than the release commit**, and the reason is the block's own rule: it reconciles three registers *counted, never incremented* — CHANGELOG headings, `git ls-remote --tags`, and this file's table — and at the release commit the tag does not exist yet, so writing it there means incrementing, which is the exact failure the block records. Write it when the index has been read, in the same commit as STATUS's two published-version registers. Clarified 20260903 at 0.32.2, because the practice had split and this row settled neither half: **0.32.1 wrote the block only in its post-publish sweep** (`efa73d1`), while **0.32.0 wrote it in both** — the release commit `0239c47` and the sweep `a981b2c` — so the same paragraph was edited twice for one release, the first time from numbers that could not yet be counted. This sentence was itself drafted as *"every release since 0.31.0 had done it that way"* and was false; reading the three commits is what found that, which is the rule this file applies to every other claim |
