...
**Latest release: 0.32.6** · last reviewed 20260904 15:13
...
| `pnk sync` | shipped | `--rebuild`, `--scan-links`, `--sidecars-only`, `--index-only`, `--extract`, `--force`, `--clear-cache` (bare, `=all`, `=paid`, `=transcripts`). **0.32.2:** `--sidecars-only --index-only` is refused rather than silently writing into `docs/`; an ordinary deletion no longer reports a move and a mint; and a document you repair clears its own row in the failure ledger instead of being listed forever. **0.32.3:** it no longer crashes on Python 3.13 for an unreachable symlink, a mistyped `--source-type` is a usage error, and an empty KB stops blaming filters nobody passed. **0.32.4:** a source directory the walk cannot enter now **holds** every document under it instead of deleting them — the run names the directory, offers `chmod +rx`, and exits non-zero, where it used to report `N removed` at exit 0 |
...
| **0.32.4** ✅ | **A source directory the walk could not enter deleted every document under it, silently, at exit 0.** `pnk sync` printed `N removed`, dropped their chunks, and `pnk doctor` reported OK — the same trade [0.32.0](#) refused for a single unreadable *file*, arrived at from the other direction and missed. **Three shapes reach it and they are not one mechanism.** A root that is a symlink into a blocked in-KB ancestor crashes `is_dir()` on Python 3.13 and returns `False` on 3.14. A `0o000` subdirectory is silent on **both**, so that shape is not a floor defect at all — the loss happens where `root.glob(pattern)` yields nothing and `pair()` reasons from absence. And a `0o400` subdirectory — **listable but not traversable** — is the one no error hook can see: `scandir` succeeds so `os.walk`'s `onerror` never fires, the glob yields the entry by name, and every `stat` on it then fails. `src/pinakes/paths.py` gains `is_directory`, `is_symlink`, `unreachable` and `unreadable_directories`; the hooked walk runs **beside** the glob rather than replacing it, because reimplementing glob semantics to gain an error hook is a larger risk than the defect; and the *parent* of any unreachable candidate is recorded, which is what covers the third shape. **An `exclude` pattern deliberately does not suppress it** — the walk is refused before any pattern applies, and `docs/sub/**` does not match the directory anyway, so suppression would be unreliable as well as unsafe. Measured on a 30-document KB: `0 removed`, `30 unchanged`, exit 1, and `pnk search` still answers from the held document inside the locked directory. **No `schema_version` bump and no rebuild** |
| **0.32.5** ✅ | **Three commands stopped reporting a permission problem as something else.** One condition again — a document or a partner KB that is **present but unreachable** — and three different wrong answers. `pnk link` blamed the user's own path, printing *“is not a document in this KB”* with a remedy about spelling. `pnk doctor` dropped an unreadable paid-extracted document from **both** its stale and its unreadable lists, so the safeguard reported `none` about a document nothing could read. And a linked partner KB that was merely unreadable came back as **absent** — *“no such directory”*, indistinguishable from one that was never created, across `pnk doctor`, `pnk link` and the cross-KB scan. **All three were the same dead `except OSError`**: the probe raises on Python 3.13 and returns `False` on 3.14, so the guard fired on the floor and nowhere else. `paths.unreachable_through_links` answers the question those callers actually have — *did the filesystem refuse, as opposed to the thing not being there* — and `why_not_a_kb` is total, so three call-site guards that could no longer fire were removed rather than left as comments justifying dead code. **The developer tooling half:** `tools/land.py` now refuses to land a **merged** tree that no `./check.sh` run has certified, with no override and `--cleanup-only` exempt; and `tools/mutate.py` names the interpreter its counts came from, since a battery inherits whatever interpreter its venv happens to hold and *“64/64 killed”* is otherwise an interpreter-dependent claim that reads as an absolute one. **Neither ships in the wheel.** **No `schema_version` bump and no rebuild** |
| **0.32.6** ✅ | **Three instruments could report success while measuring nothing, and one register had never been compared to what it described.** The vacuous-injection audit now refuses an empty site list (`--min-sites`, default 10) instead of printing `0 sites · 0 vacuous · 0 not ruled`, calls a probe run in which nothing passed `INCONCLUSIVE` rather than a verdict, and marks a run whose environment changed mid-flight `UNATTRIBUTABLE` at non-zero exit; it also runs on **Linux** now, under the declared floor, naming the interpreter, the platform and `NAME_MAX` beside its counts — because whether an injected `ENAMETOOLONG` is redundant is a property of the filesystem, not of the code. A `pnk doctor` test asserted only that the diagnosis *finished*: it passed with its own injection disabled, so it would have gone on passing the day the production code stopped calling `Path.read_bytes`. And `tools/register_gate.py` compares a documented row count to the file it names — written because this repo's own harvest `README.md` stated **three different counts for one dataset**, the one under its own steering heading being a figure withdrawn twice and 4.4x too high. **Only two things here reach a user**: `prices.toml` carries the 2026-09-04 fixing (`1.1615` → `1.1622`, EUR figures 0.06% apart for the same USD cost), and two `pathlib` holdouts in `sync.py` and `upgrade.py` became version-independent spellings — **hygiene, not a fix**, on paths no production call can currently deliver, measured rather than assumed. The audit tooling and the gate **ship in no wheel**. **No `schema_version` bump and no rebuild** |
...
**0.32.3, and the first release here whose hold was lengthened on purpose, 20260903 15:10:** `uvx --no-cache --refresh --from "pinakes[light]==0.32.3" pnk --version` prints `pinakes 0.32.3` — from the index, on the first attempt. `https://pypi.org/simple/pinakes/` carries `pinakes-0.32.3-py3-none-any.whl` and `pinakes-0.32.3.tar.gz` and enumerates **fifty-five** versions, **counted from the index rather than incremented from fifty-four**. `gh release view v0.32.3` reports non-draft, non-prerelease, created 15:09:38Z and published 15:10:50Z; the wheel upload is stamped 15:10:46Z and the sdist 15:10:48Z. **Two controls, and both fire against the published artifacts rather than against a version string.** On 3.14, against a KB holding a document that is present but unreachable: published **0.32.3** reports `OK orphaned sidecars: none`, published **0.32.2** reports `WARN orphaned sidecars: 1: docs/alias.md.pnk.yaml` beneath the `--prune` remedy — the defect this release exists to remove, still visible in the artifact before it. On 3.13, same KB: **0.32.3 exits 0** and **0.32.2 exits 1** on a raw `PermissionError`. **The hold ran 6 minutes 4 seconds** — landed 15:04:42, wheel on the index 15:10:46 — against 47 seconds for 0.32.2, and **the difference is deliberate**: the tag was held until `minimum-python` had completed at the release commit and been read **in its log**, where CPython 3.13.15 was downloaded, the assertion step printed `3.13.15`, and pytest reported **2425 passed, 126 skipped**, the same counts as the local 3.13 run. A gate that has run once elsewhere is not a gate that has run here: this job triggers only on a push to `main`, so every release commit gets its own first run.

**0.32.4, whose control is the release's own defect still visible in the wheel before it, 20260903 16:53:** `uvx --no-cache --refresh --from "pinakes[light]==0.32.4" pnk --version` prints `pinakes 0.32.4` — from the index, first attempt. `https://pypi.org/simple/pinakes/` carries `pinakes-0.32.4-py3-none-any.whl` and `pinakes-0.32.4.tar.gz` and enumerates **fifty-six** versions, **counted from the index rather than incremented from fifty-five**. `gh release view v0.32.4` reports non-draft, non-prerelease, created 16:49:09Z and published 16:53:15Z; the wheel upload is stamped 16:53:12Z and the sdist 16:53:13Z. **The control fires on the published artifacts, against one KB with two indexed documents and a `chmod 000` source root:** published **0.32.3** prints `2 removed` and exits **0**; published **0.32.4** prints `2 unchanged, 0 removed`, names the directory — *“failed: docs: directory could not be entered: Permission denied.”* — and exits **1**. That is the defect and its fix read out of two wheels rather than out of two version strings. **The hold ran 4 minutes 5 seconds** — landed 16:49:07, wheel on the index 16:53:12 — with the tag held until `minimum-python` had completed at *this* commit and been read in its log: CPython 3.13.15, the assertion printing `3.13.15`, **2454 passed, 126 skipped**. **The simple index reported zero files for the first minutes** while the install from that same index had already succeeded — the third consecutive release on which one endpoint's silence would have reported a failure that had not happened.

**0.32.5, whose control is a message rather than a crash, 20260904 09:01:** `uvx --no-cache --refresh --from "pinakes[light]==0.32.5" pnk --version` prints `pinakes 0.32.5`. `https://pypi.org/simple/pinakes/` carries both artifacts and enumerates **fifty-seven** versions, **counted from the index rather than incremented from fifty-six**. **The control discriminates the fix, and it had to be read in full rather than grepped**: against a KB whose source sits under a `0o000` directory, published **0.32.4** answers `error: 'docs/note.md' is not a document in this KB.` and published **0.32.5** answers `error: 'docs/note.md' cannot be read: Permission denied.` with a remedy naming the directory's permissions. **Both exit 1** — only the message moved, which is what made the change safe in a PATCH, and a first pass that grepped for `error|cannot` reported *nothing* for 0.32.5 because it matched a different line. **The hold ran 5 minutes 17 seconds** — landed 08:55:41, wheel on the index 09:00:58 — with the tag held for `minimum-python` at *this* commit, read in its log: CPython 3.13.15, **2474 passed, 126 skipped**, and the whole run green before the tag rather than that one job alone. **The install reported `unsatisfiable` on the first attempt** while both artifacts were already on the simple index — the fourth consecutive release on which one endpoint, read once, would have reported a failure that had not happened.

**0.32.6, whose control is two published wheels read side by side, 20260904 15:11:** `uvx --no-cache --refresh --from "pinakes[light]==0.32.6" pnk --version` prints `pinakes 0.32.6`, installed from the index. `https://pypi.org/simple/pinakes/` carries both artifacts and enumerates **fifty-eight** versions, **counted from the index rather than incremented from fifty-seven**. **The control is two-sided, which is what makes it a control:** the published 0.32.5 wheel's `pinakes/budget/prices.toml` carries `usd_per_eur = "1.1615"` and the published 0.32.6 wheel's carries `"1.1622"` — read out of both artifacts, not out of this repository. Both wheels hold **78 entries and no `tools/` entry**, which is the inverted half: `register_gate.py` and the injection audit are developer tooling and were verified *absent* from what shipped. **The hold ran 6 minutes 55 seconds** — landed 15:04:43, wheel on the index 15:11:38 — with the tag held until `minimum-python` had completed at *this* commit and been read in its log: **CPython 3.13.15 downloaded, 2510 passed, 126 skipped**. **The release's own subject ships in no wheel**, so for the first time in this list the artifact check is mostly an absence check, and the one thing a user can observe — the ECB rate — is the only thing there was to find inside.
