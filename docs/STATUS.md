...
| Deep paid client | **on `main`, unreleased**, unreachable | E3. `src/pinakes/deep/client.py` — the **second and final** entry on `.paid-path-allowlist`, and the module that builds a round's two calls: decompose, and answer. Nothing reaches it: no CLI path constructs a transport, so it cannot spend. Two structural defences ship with it — a subproblem is a plain string because the schema has no other field it could be, and an answer cites **passage numbers**, so a citation naming evidence the call never had is refused. `pnk serve` never loading it is now a gate. `src/pinakes/paid.py` holds what both paid clients obey; it is deliberately **not** allowlisted, because it imports no client |
| Deep-round estimator | **on `main`, unreleased**, inert | E2. What one `pnk ask --deep` would cost, before the first call: the cheap branch's single synthesis call, a loop round, and `max_rounds x` a round for the whole operation. Pure — no client, no I/O, no wall clock — and **nothing calls it**, so nothing can spend. Unreleased deliberately: no user-visible surface until `--deep` exists, so it cuts with E4, which is what first makes `--deep` real. At the shipped defaults the cheap branch is EUR 0.2627 and a five-round loop EUR 2.81, against a default `per_operation_eur` of 0.30 |
