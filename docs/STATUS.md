...
**Latest release: 0.32.0** · last reviewed 20260902 10:25
...
**This release's CI went red *after* its artifact was verified**, which is the ordering worth recording. `fe58be3`'s `check (light pdf claude)` leg fails in `tests/test_pdf_trace.py`: refreshing `usd_per_eur` to `1.159` separated two routes to the same euro amount that the previous rate had made equal by coincidence — `(a/r)+(b/r)` against `(a+b)/r`, differing in the 28th digit. Nothing published is wrong and the money error is 1E-28 EUR, but **a verified artifact and a green `main` are two claims, and this release has only the first**.
