...
| Deep-round estimator | **on `main`, unreleased**, inert | E2. What one `pnk ask --deep` would cost, before the first call: the cheap branch's single synthesis call, a loop round, and `max_rounds x` a round for the whole operation. Pure — no client, no I/O, no wall clock — and **nothing calls it**, so nothing can spend. Unreleased deliberately: no user-visible surface until `--deep` exists, so it cuts with E3 or E4. At the shipped defaults the cheap branch is EUR 0.2627 and a five-round loop EUR 2.81, against a default `per_operation_eur` of 0.30 |
