...
| …and the gate cannot report a pass it did not earn | fix | `tests/test_wheel_import_gate.py::test_an_allowance_covers_only_the_module_it_names_even_for_the_same_library` + `::test_a_required_module_may_not_also_be_allowed_to_fail` + `::test_a_package_outside_site_packages_is_refused` + `::test_a_package_with_no_file_is_refused_rather_than_resolved_to_the_cwd` |
| a freshly-resolved install answers an MCP handshake, before publishing | fix | CI `build` + `release.yml`, pinned by `tests/test_check_script.py::test_ci_drives_a_real_mcp_handshake_against_a_freshly_resolved_install` + `::test_the_release_workflow_exercises_the_wheel_it_is_about_to_publish` |
...
| the six ordered release sequences in `CHANGELOG.md`, `docs/ROADMAP.md` and `docs/STATUS.md` read in release order | fix | `tests/test_release_order_gate.py::test_the_real_documents_are_in_release_order`, `tests/test_release_order_gate.py::test_an_ordered_tree_passes` |
