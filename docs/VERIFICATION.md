...
| a mistyped `--questions` is an argument error at argparse time, not a traceback after a corpus build — `--fake` syncs a whole temporary corpus before any questions file is opened, so the check has to precede it. A directory is refused too (`.is_file()`, not `.exists()`), and the KB's own default is deliberately left unguarded: an absent `<kb>/eval/questions.yaml` is a corpus that cannot be measured rather than a mistyped argument | fix | `tests/test_eval.py::test_a_mistyped_questions_path_is_refused_before_the_corpus_is_touched` |
