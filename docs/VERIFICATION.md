...
* **The scope began as `plans/20260727_1543-v0.2.md`'s promises**, which is what the table this
  ...
  measured a different tree. **Count them instead**, with the gate's own filter — and note that the rule reaches *inside* a
row too: one said *“its four hard-won cases”* and then listed six, from 20260903 until a planner
read the row against its own list rather than against its claim.
...
| a paid document **retired and restored byte-identical** is refused with the reason `RETIRED`, never *content changed* — `SoftDelete` drops its chunks so the re-extraction is genuinely needed, but the file did not move a byte and saying it did asks the user to pay for a change that never happened. `CHANGED` still wins when the file also came back different, which is the stronger true statement and the half an obvious implementation inverts | fix | `tests/test_pairing.py::test_a_retired_paid_row_whose_file_returns_unchanged_says_retired_not_changed`, `tests/test_pairing.py::test_a_retired_paid_row_whose_file_returns_changed_still_says_changed`, `tests/test_sync.py::test_a_retired_paid_document_restored_unchanged_is_not_told_its_content_changed` |
...
| the probe measures the golden set `--questions` names, rather than the one its KB happens to hold — `tools/build_rfc_corpus.py` rewrites `<kb>/eval/questions.yaml` on every build, so until the flag existed there was no route to re-measure the set a rebuild replaced. The question **count** is what discriminates: a probe reporting the flagged path while loading the default satisfies every assertion about `path` and `sha256`, which are computed from the path rather than from what was read | fix | `tests/test_eval.py::test_the_probe_measures_the_golden_set_the_flag_names` |
| a relative `--questions` is recorded resolved, so two sets both reached as `./kept.yaml` from two directories are told apart in the artifact — the same property `kb_root` already has, and it needs a run whose *cwd* makes the argument relative to pin it | fix | `tests/test_eval.py::test_a_relative_questions_path_is_recorded_resolved` |
| a refusal names the golden set that was read, never the default it displaced — `check_measurable` is handed `source=` separately from the questions themselves, so pointing the loader at the flag and leaving `source` on the default is a half-implementation that fails in the one direction that matters: it sends an operator to fix a file that did not refuse | fix | `tests/test_eval.py::test_a_refusal_names_the_golden_set_the_flag_named` |
| a `--questions` path that cannot be read is an argument error at argparse time, not a traceback after a corpus build — `--fake` syncs a whole temporary corpus before any questions file is opened, so the check has to precede it. Two arms: `.is_file()` refuses a missing path, a directory, and anything else `.exists()` would admit and `read_text` would then block on; a trial open then refuses a regular file the process cannot read, reporting the OS's own `strerror`. The KB's own default is deliberately left unguarded — an absent `<kb>/eval/questions.yaml` is a corpus that cannot be measured rather than a mistyped argument. The unreadable case is **injected, not chmod'd**, through a `sitecustomize.py` on the child's `PYTHONPATH` | fix | `tests/test_eval.py::test_a_mistyped_questions_path_is_refused_before_the_corpus_is_touched` |
...
**Since 20260831 the failure it produces is *contained* rather than resolved**: the collision is
caught narrowly in `sync._apply` and recorded as a `PathStillHeldError` with a remedy, instead of
escaping as a raw `sqlite3.IntegrityError`. The plan is still inapplicable and `pnk sync` still
exits non-zero — only what the user is told has changed.
...
| **a chain whose earlier member fails to index records the collision instead of crashing** — the ordering pass makes action N+1 depend on action N, and a caught failure *rolls back*, so the failed row keeps its path for the next action to land on. Reached through `_apply`'s own "failures are recorded, the run continues" path, so any caught class gets there: a non-UTF-8 byte, a missing extractor, an `OSError`, a budget refusal | fix | `tests/test_sync.py::test_a_rename_chain_whose_middle_document_fails_records_the_collision_instead_of_crashing` |
| **a cycle is a recorded failure with a remedy, not a traceback** — contained, not resolved: rows are still left at paths no longer on disk, and `pnk sync` still exits non-zero. **Both this row and the one above assert `not report.ok`**, because catching an exception trades a crash for something quieter by construction and a contained failure that exited 0 would be worse than the traceback it replaced | fix | `tests/test_sync.py::test_a_rename_cycle_is_a_recorded_failure_with_a_remedy_rather_than_a_traceback` |
| **only the `documents.path` collision is caught, and every other constraint still escapes** — `sqlite3.IntegrityError` also covers the `chunks(doc_id, ordinal)` and `nodes(kind, key)` UNIQUEs, the `links`/`edges` primary keys and the CHECKs on `documents.state`, `links.origin` and `nodes.kind`, each of which fires only when Pinakes is wrong. **The `NOT NULL constraint failed: documents.path` case is load-bearing**: it is the only breach naming the right column under the wrong error code, so without it the three other negative cases all fail on the column substring and the `sqlite_errorname` clause goes unexercised | fix | `tests/test_sync.py::test_only_the_documents_path_collision_is_caught_and_every_other_constraint_escapes` |
| the same narrowing witnessed through `_apply` rather than the predicate alone — an injected CHECK breach re-raises instead of being recorded. **A seam, and named as one**: no fixture can reach a CHECK breach from a KB on disk, so this proves the handler re-raises and not that the constraint is reachable; the reachable half is covered end to end by the two rows above | fix | `tests/test_sync.py::test_an_integrity_error_from_another_constraint_still_escapes_apply` |
| `_vacates` names **every** way a path stops being held, not the subset today's plans exercise — a move frees the path it came from, and a retirement frees the path it retires, because `sync`'s write guard deletes a *retired* row holding a path a different id now wants. Pinned on the helper rather than through a plan: the retirement case is unreachable through `pair()`, so no fixture reaches it and the battery row for that branch declared a `kills` selector while its prose said it was expected to survive | fix | `tests/test_pairing.py::test_vacates_reports_every_way_a_path_is_freed` |
...
| a version that is landed but not published — `line3 > R`, the newest entry of STATUS's *Published versions* row — must carry the hold marker, on a page that deploys on every push | fix | `tests/test_status_header_gate.py::test_a_landed_but_unpublished_version_requires_the_hold_marker`, `tests/test_status_header_gate.py::test_the_hold_marker_satisfies_layer_two_when_it_names_the_published_version` |
| a version that **is** published may not keep the marker — the half X7's index rule is green on by construction, and no release-order sequence reads line 3's tail | fix | `tests/test_status_header_gate.py::test_a_published_version_may_not_keep_the_hold_marker` |
| the marker is a parsed shape carrying a checkable claim, not a glyph — its qualifier must name the version `pip install` actually gets | fix | `tests/test_status_header_gate.py::test_a_hold_marker_naming_the_wrong_published_version_fails`, `tests/test_status_header_gate.py::test_the_marker_must_be_a_parsed_shape_not_a_loose_glyph` |
| the qualified form of line 3 is **legal** — `SHAPE` has no `$`, so it passed by accident and tightening the regex would have outlawed the sanctioned state silently | fix | `tests/test_status_header_gate.py::test_the_qualified_form_is_legal_and_that_is_now_pinned` |
| a headline behind the published row is always red | fix | `tests/test_status_header_gate.py::test_a_headline_behind_the_published_row_is_always_red` |
| a *Published versions* row the gate cannot read is a **hard failure, never a skip** — missing, or no longer unique | fix | `tests/test_status_header_gate.py::test_a_row_the_gate_cannot_read_is_a_hard_failure_never_a_skip`, `tests/test_status_header_gate.py::test_a_second_published_versions_row_fails_rather_than_reading_the_first` |
| `R` is read through `release_order_gate.SEQUENCES` rather than a second copy of its `within` anchor, and a rename there fails rather than disabling the layer | fix | `tests/test_status_header_gate.py::test_the_published_row_sequence_still_resolves`, `tests/test_status_header_gate.py::test_a_renamed_sequence_fails_rather_than_disabling_layer_two` |
| layer 1 stays first, so `ci.yml`'s negative check still reads the reason it greps for | fix | `tests/test_status_header_gate.py::test_layer_one_runs_first_so_the_ci_negative_check_still_reads_its_reason` |
...
| `make release-check` refuses when **no** release tag points at `HEAD` — the half the target never had, for its whole life | fix | `tests/test_release_tag_gate.py::test_no_tag_at_head_fails`, `tests/test_release_tag_gate.py::test_a_tag_that_is_not_release_shaped_does_not_count_as_a_tag` |
...
| a `retro.d/` fragment carries its own `## ` heading, so it cannot be **absorbed** — `render` joins bodies with a blank line, and prose with no heading of its own lands under whichever fragment sorts before it and reads as that incident's lesson. The rule is in `check`, which reads the fragments going in, because the assembled document is *correct markdown* and a checker reading the result is reading the evidence after it was destroyed — pinned by asserting `document_problems` returns nothing on the very assembly `check` refuses. **The separator is exactly one space** — `^## \S`. `##  A lesson` renders correctly as a heading and is refused anyway: the gate is deliberately stricter than the renderer, because the fragments are written to one spelling and a second one reaching the document is a difference nobody chose. That was accepted when this row was first written and is not now, which is why the row moved with the code. | fix | `tests/test_fragments.py::test_a_retro_fragment_with_no_heading_of_its_own_is_refused`, `tests/test_fragments.py::test_a_headingless_fragment_is_invisible_to_the_checker_that_reads_the_document`, `tests/test_fragments.py::test_a_third_level_heading_does_not_open_a_fragment`, `tests/test_fragments.py::test_the_separator_is_one_space_and_the_gate_is_stricter_than_the_renderer` |
| that heading's `(YYYYMMDD HH:MM)` equals the filename's own prefix — a copy, never a second reading of the clock, which is a second chance to be wrong (20260826: three headings typed from memory in one morning, out by 1 minute, 2 minutes and 3 hours 30 minutes). Nothing looser is accepted — not the date alone, not an em-dash for the parentheses, not a trailing `UTC` — because a gate accepting three spellings is checking that somebody typed a date, not that the stamp is the filename's. A fragment whose name carries no `YYYYMMDD_HHMM-` prefix has none to copy, so it owes the heading and not the stamp — the exemption is keyed on that prefix being absent, not on the file's age. **And the stamp must *end* the heading, not merely appear in it** — `[(20260901 07:10)]`, `x(20260901 07:10)x` and `( (20260901 07:10) )` all carry it as a substring and are all refused. What licenses anchoring it there is a count of the corpus, not a preference: of `docs/RETROSPECTIVES.md`'s **148** `## ` headings **at `0aea036`**, **136** carry a parenthesised stamp and **0** carry one anywhere but the trailing position | fix | `tests/test_fragments.py::test_a_heading_whose_stamp_disagrees_with_the_filename_is_refused`, `tests/test_fragments.py::test_nothing_looser_than_the_ruled_stamp_is_accepted`, `tests/test_fragments.py::test_a_fragment_with_no_prefix_owes_a_heading_and_not_a_stamp` |
| the two are separate problems with separate fixes and are reported separately, and neither rule reaches `changelog.d/`, whose fragments are `- ` bullets merged under a category heading `render` synthesises for them. **The live corpus cannot stand in for these fixtures**: `retro.d/` holds only *unreleased* fragments — the rest are spliced into `docs/RETROSPECTIVES.md` and deleted at each release — so a green gate says nothing about the released corpus, and the only live counter-example when this was built was a fixture inside the test suite | fix | `tests/test_fragments.py::test_a_fragment_that_is_neither_reports_both_problems`, `tests/test_fragments.py::test_the_heading_rule_never_reaches_the_changelog_stream` |
| a refused fragment is told the **cause**, never a symptom — this arm exists because a message sending the writer to fix what is already correct is worse than none. A byte-order mark is named as a mark, not reported as a missing heading; a mark alone on line 1 does not also produce a false missing-heading and missing-stamp about the correct heading two lines below it; and a heading hidden behind leading whitespace is quoted **with** that whitespace, not stripped of the one character that caused the refusal. **And a whitespace-only first line is not read as the opening** — a fragment whose heading sits on line 2 behind a line that looks blank and is not is **accepted in silence** — exit 0, no messages at all: the blank-looking line is skipped and the heading and stamp on line 2 are the ones read, so the writer is told nothing, neither about the whitespace nor that it has neither heading nor stamp, the latter being what would send them to fix a heading that is already correct | fix | `tests/test_fragments.py::test_a_byte_order_mark_is_named_rather_than_reported_as_a_missing_heading`, `tests/test_fragments.py::test_a_byte_order_mark_alone_on_the_first_line_reports_only_itself`, `tests/test_fragments.py::test_a_heading_hidden_by_leading_whitespace_is_quoted_with_the_whitespace_showing`, `tests/test_fragments.py::test_a_whitespace_only_first_line_is_not_read_as_the_opening`, `tests/test_fragments.py::test_the_stamp_arm_reads_the_heading_and_not_whatever_line_is_first` |
...
| a release that was **prepared but reached no index** is declared absent from the two lists recording what PyPI serves, and from **nothing else** — 0.30.3 stays expected in CHANGELOG's headings and link definitions, ROADMAP's table and sections, and STATUS's release roadmap, because it is a real release document and only never a published artifact. Lag explains a missing newest; only a declared absence explains a missing middle, and the allowance is printed on the green run because a tolerated gap and a declared one are identical from an exit status | fix | `tests/test_release_order_gate.py::test_0303_is_declared_absent_from_the_published_lists_and_from_nothing_else`, `tests/test_release_order_gate.py::test_a_release_that_reached_no_index_is_declared_absent_rather_than_tolerated`, `tests/test_release_order_gate.py::test_removing_that_declaration_turns_both_published_lists_red_and_nothing_else` |
...
| the injection audit names the **interpreter, platform and `NAME_MAX`** its probes ran under, beside the counts rather than above the table, and says so plainly when it cannot — a `VACUOUS` verdict can mean *the real environment does it anyway*, so a report that cannot name its environment cannot state its own limit | — | `tests/test_vacuous_injection_audit.py::test_the_environment_is_asked_of_the_launcher_not_of_this_process`, `tests/test_vacuous_injection_audit.py::test_the_probe_runs_the_same_launcher_the_probes_do`, `tests/test_vacuous_injection_audit.py::test_an_environment_the_run_cannot_establish_is_said_so_never_guessed`, `tests/test_vacuous_injection_audit.py::test_a_probe_that_exits_non_zero_is_unknown_not_its_stdout`, `tests/test_vacuous_injection_audit.py::test_the_environment_line_sits_beside_the_counts_not_above_the_table`, `tests/test_vacuous_injection_audit.py::test_an_unidentifiable_environment_says_the_verdicts_cannot_be_read_without_it` |
| the audit exits **non-zero on the unruled set** — `UNSTABLE`, `INCONCLUSIVE`, no caller found — and **zero on `VACUOUS`**: the instrument failing to rule is the failure, while a vacuous row is a finding a person splits three ways, for the same reason `mutate.py` exits 0 on a survivor | — | `tests/test_vacuous_injection_audit.py::test_a_vacuous_row_is_reported_and_exits_zero`, `tests/test_vacuous_injection_audit.py::test_a_site_the_instrument_could_not_rule_exits_non_zero`, `tests/test_vacuous_injection_audit.py::test_a_site_with_no_caller_found_exits_non_zero_too`, `tests/test_vacuous_injection_audit.py::test_an_all_sound_run_exits_zero` |
| a probe run in which **nothing passed** is `INCONCLUSIVE`, never a verdict — `1 skipped` exits 0 and contains neither `no tests ran` nor `0 passed`, so an all-skipped selector used to read as `VACUOUS`, which is the one direction the tool must not fail in because a vacuous row is what a person then acts on | — | `tests/test_vacuous_injection_audit.py::test_a_run_that_passed_nothing_is_never_read_as_a_verdict`, `tests/test_vacuous_injection_audit.py::test_a_real_always_skipped_test_reports_inconclusive_end_to_end` |
| a **collapsed site list is refused**, not reported clean — `TARGET` is a regex over source lines, so a reformat that wraps a `monkeypatch.setattr` across two lines empties the collection and `0 sites · 0 vacuous · 0 not ruled` exits green while measuring nothing. The floor is asserted **by value**, `--min-modules 1` being the sibling gate's own lesson | — | `tests/test_vacuous_injection_audit.py::test_a_collapsed_site_list_is_refused_rather_than_reported_clean`, `tests/test_vacuous_injection_audit.py::test_the_floor_is_a_value_not_a_flag`, `tests/test_vacuous_injection_audit.py::test_the_floor_can_be_disabled_outright` |
| the audit's own test file **stays out of the audit's population** — `TARGET` greps source lines, so an injection written out as fixture data made this file a site, and probing a line inside a string literal broke the file and returned `INCONCLUSIVE`: 15 sites became 17 and the run went red over a site that does not exist | — | `tests/test_vacuous_injection_audit.py::test_this_file_does_not_add_itself_to_the_audits_own_population` |
| the environment is read **before** the probe loop and again after, the report names the **before** reading, and a run whose environment moved is `UNATTRIBUTABLE` and non-zero — asking at the end reports the environment the run *finished* in, and this venv answered 3.13.15 to the audit and 3.14.7 to the next command in the same directory | — | `tests/test_vacuous_injection_audit.py::test_the_environment_is_read_before_the_probes_not_after_them`, `tests/test_vacuous_injection_audit.py::test_an_environment_that_moved_mid_run_makes_every_verdict_unattributable`, `tests/test_vacuous_injection_audit.py::test_a_stable_environment_is_not_reported_as_drift` |
...
| a fragment's links resolve from the document its body is **spliced into**, not from the directory the file sits in — so a sibling named by filename is refused on the branch instead of at the release cut, where it had already reached `main` twice, and the `#…` anchor form both fragment READMEs prescribe is accepted. The anchor universe is the destination's headings plus every pending fragment's, and a slug two files both contribute is refused rather than guessed at. The two streams splice to different depths, so `changelog.d/` resolves from the repository root and `retro.d/` from `docs/` | fix | `tests/test_markdown_link_gate.py::test_a_fragment_linking_a_sibling_by_filename_is_refused_before_anything_is_spliced`, `tests/test_markdown_link_gate.py::test_a_fragments_anchor_into_a_sibling_fragment_resolves`, `tests/test_markdown_link_gate.py::test_an_anchor_two_files_both_contribute_is_refused_rather_than_guessed_at`, `tests/test_markdown_link_gate.py::test_a_changelog_fragment_resolves_from_the_repository_root_not_from_its_directory` |
...
`StrictUndefined`. **That fourth one is narrower than it first reads, and the review pass
measured how narrow.** `finalize` is handed the undefined itself, and almost anything a hook
does with it re-raises: `str()` and a truthiness test both raise `UndefinedError` on it, so
only a hook that returns something *without ever touching* the undefined — a constant, a
sentinel, a caught exception — hides the raise. A test that only proves the escaper fires
cannot catch any of the four.

| What must be true | Increment | Where it is checked |
|---|---|---|
| a name holding any of the eleven values that open out the **three classes** the sweep named — `"`, `\`, and control characters other than tab — renders a manifest `tomllib` parses **and reads back as the name that went in**: parsing is the weaker claim, and a manifest can parse while holding the wrong name. The eleven are quote, backslash, both together, newline, carriage return, NUL, bell, backspace, form feed, unit separator and DEL — of which `C:\notes\kb` is the **verifier's** widening, not the sweep's, and *both together* is a pair rather than a character | S4 | `tests/test_template.py::test_a_name_in_any_class_the_sweep_named_renders_a_manifest_that_parses` |
| the same eleven values survive `pnk init` and come back out of `load()` — the defect reported success, so a test that stops at `render_manifest` is looking at the half that was never broken | S4 | `tests/test_template.py::test_init_writes_a_kb_that_load_can_open_whatever_the_name_holds` |
| a backslash path that stays **valid** TOML still reads back unchanged — `C:\notes` holds only `\n`, which TOML accepts, so an unescaped manifest parses cleanly and means `C:` + newline + `otes`, and `pnk doctor` calls that KB healthy under a name nobody typed. The row above cannot see it: **both** of its backslash values carry a sequence TOML rejects outright — `\k` in `C:\notes\kb`, and `\a` in `C:\a"b\\c` — so with the escape removed each dies on the parse before the round-trip is ever reached. **The illegal character is not the same one in both, and naming only `\k` was the third claim in this increment asserted without being run.** **Found by running the mutation battery and reading the kill reason** | S4 | `tests/test_template.py::test_a_backslash_path_that_stays_valid_toml_still_reads_back_unchanged` |
...
| a variable this build does not supply still raises a message with a remedy — `finalize` runs *in front of* `StrictUndefined` and is handed the undefined itself. The regression to guard is **narrow and measured**: a hook that stringifies or tests truthiness re-raises anyway, so only one that returns without touching the undefined hides it | S4 | `tests/test_template.py::test_a_variable_this_build_does_not_supply_still_raises_a_message_not_a_traceback` |

**What these rows do not cover, named rather than implied.** Escaping makes a value safe inside
a TOML **basic** string — where a value *has* an escaped form at all. Where it does not, the
row above says the answer is a refusal rather than an escape, and that class was found by a
review pass and not by the corpus: every value the corpus holds was chosen by someone looking
