...
| `make release-check` runs the gate as the command whose exit status `make` reads, and its `##` help string cannot promise a check the recipe does not run | fix | `tests/test_release_tag_gate.py::test_make_release_check_runs_the_gate_rather_than_describing_it`, `tests/test_release_tag_gate.py::test_the_release_check_help_string_no_longer_promises_more_than_it_does` |
