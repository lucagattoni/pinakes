...
| the five ordered release sequences in `CHANGELOG.md`, `docs/ROADMAP.md` and `docs/STATUS.md` read in release order | fix | `tests/test_release_order_gate.py::test_the_real_documents_are_in_release_order`, `tests/test_release_order_gate.py::test_an_ordered_tree_passes` |
...
| **no confidence branch, and no surface, ever prints `--deep`** — the flag lands in E4, and one that parses and then apologises is the defect `0.20.1` fixed for `vector_tier` | E1 | `tests/test_cli_ask.py::test_no_confidence_branch_ever_prints_a_flag_that_does_not_exist`, `tests/test_cli_ask.py::test_the_deep_flag_is_a_usage_error_rather_than_an_apology` |
