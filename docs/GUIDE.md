...
| Searches slow past ~50k chunks | NumPy tier is exact, not sublinear | Expected; `pnk doctor` warns. The `sqlite-vec` tier is the template release — **setting `vector_tier = "sqlite-vec"` is refused, not a workaround.** Splitting the KB is the honest answer |
