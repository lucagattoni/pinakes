- **A symlink that resolves to nothing is reported by `pnk sync` rather than skipped in silence.**
  ...
  and says the target is missing, unreadable, or looping — three causes rather than two, because
  `exists()` returns False for a target this process may not reach and cannot tell you which of
  the three it hit. A symlink to a real **directory** is an ordinary alias and is not reported; a
