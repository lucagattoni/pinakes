- **The deep release's estimator: what one `pnk ask --deep` would cost, before the first call**
  ...
  reporting a defect. Nothing is wired to the CLI yet: `pnk ask`'s escalation line still prints its
  sentence without a number until the increment that has a `[deep]` section to read.
- **A round is priced as two calls, not as one input.** The plan's formula counts a round's input
  once, and a round makes two calls — so counting it once under-prices every round by one carried
  memory plus one prompt, which is the direction a budget may never be wrong in. Both calls are
...
- **`tools/measure_passage_tokens.py`** — the offline half of the measurement behind the per-passage
  ceiling. A chunk is sized in the embedding model's tokenizer and billed in the vendor's, and the
  conversion cannot be measured without spending; this measures the character width the two share.
