...
- **[`plans/20260811_1358-deep-release.md`](plans/20260811_1358-deep-release.md) — the live build order, and the only plan with unbuilt work in it.** Seven increments, E1 to E7; **all eight decisions (D-21 to D-28) were taken 20260811 14:17 and it is the authority for them.** **E1, E2 and E3 are built. Build E4 next — the loop — and nothing in the plan is undecided: D-29 and D-30 were taken 20260811 20:08 and closed the last two questions.** `[deep]` gets `model` and `max_rounds` only, settable and unstamped (`adjacent_k`'s precedent); and the default caps rise — `per_operation_eur` 0.30 → **2.00**, `daily_eur` 1.00 → **6.00**, `max_rounds` **3** — because **even a one-round loop (EUR 0.5624) exceeded the old cap**, so `--deep` would have refused on every template-stamped KB. **Raising `per_operation_eur` alone does nothing**: all three windows are checked before every call and the daily one bound first, silently. **Two consequences E4 owns**: the template's `[budget]` block changing bumps the template version (T1), and a default raise reaches only *new* KBs — every existing one stamps 0.30, so the **refusal message** is the whole remedy there. Its rejected options are kept with their costs — read them before re-opening one. **What E3 leaves E4**: three bounds are enforced in the client and *refuse* rather than trim (the question ceiling, `CARRIED_MEMORY_CHAR_CEILING`, and `final_k` passages per answering call), citations arrive as passage **numbers** so E4 owns the mapping back to documents, and `DeepCallFailedError.kind` is the vocabulary for labelling a run.
