...
- **[`plans/20260811_1358-deep-release.md`](plans/20260811_1358-deep-release.md) — the only plan with unbuilt work in it, and *nothing in it is buildable yet*.** Seven increments (E1–E7) and **eight open decisions, D-21 to D-28, every one blocking an increment.** Take them before starting E1. **Start at D-28** — it fixes the shape of the loop, and D-22 is the same question with the confidence signal missing.
  - **Two of its measurements change what the older documents imply.** The budget machinery is already built and proven by the paid extractor, so this release adds the loop and not the machinery. And **`[retrieval.confidence]` ships commented out**, so the escalation gate DESIGN §4.2 depends on does not exist on any KB a user creates — which is what most of the open decisions are about.
