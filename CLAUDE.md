...
- **[`plans/20260811_1358-deep-release.md`](plans/20260811_1358-deep-release.md) — the live build order, and the only plan with unbuilt work in it.** Seven increments, E1 to E7; **all ten decisions (D-21 to D-30) are taken and it is the authority for them.** **E1 to E4 are built — `pnk ask --deep` answers, bounded and budgeted. Build E5 next, and it is smaller than the plan says: the transcript alone (`.pinakes/deep/<operation_id>.json`, spared by the sweep, with its own `--clear-cache` target). Its other half — `pnk budget` showing `ask` operations beside `sync` ones — was verified at E4 and closed with a test: `operation` was already free-form and nothing in `budget/` moved.** Then E6, the measurement run — **the only increment that spends real money** — and E7, printed suggestions. **A cut after E4 is a complete release and the name stays in the unbuilt-work table until the final one** (D-9). **What E4 leaves E5**: `DeepAnswer` carries everything a transcript needs — the blocks, their citations resolved to documents, `stopped_by` out of `STOP_REASONS`, the estimate and the tally with its `call_ids` — so E5 serialises a value rather than re-deriving one; and `PROMPT_VERSION`/`SCHEMA_VERSION` are what a transcript must record, because what a run produced depends on what it was asked.
