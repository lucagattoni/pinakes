...
- **[`plans/20260811_1358-deep-release.md`](plans/20260811_1358-deep-release.md) — the live build order, and the only plan with unbuilt work in it.** Seven increments, E1 to E7; **all eight decisions (D-21 to D-28) were taken 20260811 14:17 and it is the authority for them.** **E1, E2 and E3 are built. E4 — the loop — is next, and it is BLOCKED on a question no plan has answered: `[deep]`'s manifest keys.** Nothing anywhere names `max_rounds`, its default, or the carried-memory key, and `_toml.py` hard-errors on an unknown key — so adding a `[deep]` section makes a manifest unreadable by every Pinakes built before it. That is a decision to take, not a choice to make; it is why E1's `cost_eur` is still `null` and why nothing is wired to the CLI. **The second thing E4 must answer**: at the shipped defaults a five-round loop prices at EUR 2.81 against a default `per_operation_eur` of 0.30, so a stock KB's `--deep` **loop** is refused at round 0 while the cheap branch fits — D-22 meeting D-23. Its rejected options are kept with their costs — read them before re-opening one. **What E3 leaves E4**: three bounds are enforced in the client and *refuse* rather than trim (the question ceiling, `CARRIED_MEMORY_CHAR_CEILING`, and `final_k` passages per answering call), citations arrive as passage **numbers** so E4 owns the mapping back to documents, and `DeepCallFailedError.kind` is the vocabulary for labelling a run.
